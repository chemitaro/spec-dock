...
- `spec-dock/active/issue/report.md` must record command evidence for required `sync`, `validate`, per-step code review, step commit / approved-no-op, final QA review, final issue-wide code review, final spec review, and final report ledger / final commit scope before the final commit, including whether each required step succeeded, passed, reached approval, or reached `committed` / `approved-no-op`.
- Do not run `issue finish` until every required step has been executed, every required `sync` / `validate` step has succeeded or passed, every required review step has reached approval or pass, every implementation step is `committed` or valid `approved-no-op`, the final report ledger and final commit scope are recorded in `spec-dock/active/issue/report.md`, the final commit is complete, no unintended staged / unstaged changes remain, and the final commit hash plus post-commit clean evidence are recorded in external delivery evidence such as the final response, PR, or issue comment.
