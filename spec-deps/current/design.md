...
- 採用しない/変更しない（理由）:
  - 既存ノードの `meta.json` の後追い更新（sync/validate での強制修正）は採用しない（スコープ外 + 予期せぬ副作用）。
...
- 追加（Add）:
  - なし
- 変更（Modify）:
  - `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/app.py`: `_write_meta()` に `_spec_dock` を追加し、read-only 化を試行する
...
- 参照（Read only / context）:
  ...
  - `spec-deps/current/adrs/adr-00001-meta-json-tool-managed-readonly.md`: 意思決定（自己記述 + read-only）の根拠
...
- EC-002 → 実装方針（sync/validate は meta を書き換えない）
...
- どのAC/ECをどのテストで保証するか:
  ...
  - EC-002 → `tests/*`（既存 `meta.json` が `sync/validate` で不変であること）
...
- EC-002:
  - Unit: `_spec_dock` を含まない既存 `meta.json` を用意し、`sync/validate` 前後で `meta.json` が不変（内容差分なし）であることを検証
...
└── spec-deps/current/
    ...
    ├── design.md                      # This
    └── adrs/
        └── adr-00001-*.md             # Read only (decision)
```
