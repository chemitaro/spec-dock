...
- `./spec-dock/scripts/spec-dock update [path]` は repo-local self-update path で、target 省略時は current directory を更新する。明示 path を渡すとその managed repo を更新する。固定6ディレクトリを丸ごと削除・コピーする。配置先のsymlinkや非ディレクトリは書込み前に拒否する
- update中は関連コマンドを停止する。失敗時は外部installerのupdateを最初から再実行する。個別ファイル保持、状態journal、自動rollbackは持たない。uninstallはツールのみを削除し、`--remove-specs` は拒否する。
