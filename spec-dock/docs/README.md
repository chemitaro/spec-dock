...
- `uninstall` の default / `--keep-specs` は managed distribution deprovision が所有する。dry-run は完全な read-only assessment であり、`--apply --keep-specs` は `spec-dock/.distribution-retry.json` schema 2 forward guard と `spec-dock/.distribution-journal.json` protocol 2 journal だけを作る。`--remove-specs` は spec history を削除し得る別の明示 authority であり、Issue 371 の compatibility owner に残す。keep と remove の recovery authority を相互に昇格しない
...
- public進行状態、最終完了段階、failed / pending path、action error、top-level error、retry policy はtyped `DistributionProcessResult`だけから生成する。CLI adapterはjournal、guard、checkpointを解釈しない。legacy `.uninstall-retry.json` はroot / mode / plan / checkpointを証明しないため自動変換しないで保持し、manual recovery guidanceだけを返す
