...
- Issue Planning authoringが三つのcanonical文書と、new-member onboarding companion guideを同一のFormal Candidate payloadとして生成・Review・Human decisionへbindするcontract。
...
| E1-REQ-022 | Issue Planning、Epic Planning、Initiative Planningへ共通のPlanning Candidate lifecycleを提供し、Scopeごとに最小packageを生成する。Issue CandidateはIssueのcanonical三文書＋exactly-one onboarding companion guide、Epic CandidateはEpic三文書＋Issue Boundary Map、Initiative Candidateは完全Portfolioを基本形とする。Issue companionはCandidate payloadであるがcanonical specificationではない。 |
...
| E1-REQ-026 | provider-owned Oracle adapter／Runtimeはidentity、reference attachment、source binding、safe artifact snapshot、hash／parity、Oracle invocation、result retrievalを所有し、Review mode／Revision lane／Human Gateのsemantic判断はSkillへ残す。Planner、Semantic Revision、archive Reviewer、git-bound Reviewerの全Formal runはRuntime→provider-owned adapter→PATH-resolved Oracle→fresh ChatGPT roleの同一境界を通り、Reviewerのclosed JSONも同じ境界を逆向きに戻す。operator-local wrapperへ製品authorityを委譲しない。 |
...
| E1-REQ-032 | compactなrole、task、authority、exact repository／named branch／HEAD、fallback禁止、output contract、Human authority boundaryをChatフォームへ送るPrompt本文に含める。operation固有の詳細はprovider-owned operation resourcesからoperation identityで選択し、operator-supplied fileを命令authorityへ昇格させない。Oracleへ渡す`--provided-context-path`はsource／evidence／prior Candidate等のopaque reference dataであり、旧`--context-manifest`や生成packへ変換しない。 |
| E1-REQ-033 | PlannerおよびSemantic Revisionの正式authoring outputは、scopeで指定されたlogical filenameとinternal rootを持ち、Issueではcanonical `requirement.md`／`design.md`／`plan.md`とRuntime指定の`artifacts/<onboarding-guide>.md`だけを収録するdownloadable ZIP artifact一個だけとする。長いinline response、markerで連結した単一text payload、patch、未宣言extra file、第四のcanonical specificationをformal authoring outputとして受理しない。Reviewerのread-only closed JSON resultは別contractとして維持する。 |
| E1-REQ-034 | provider-owned adapterはOracle sessionが保存したfile artifactを、expected／observed filename、artifact type、size、SHA-256、session identity、safe path条件へbindしてprivate stagingへsnapshotする。missing／multiple／wrong-name／wrong-root／unsafe／hash-mismatch artifactを拒否する。Prompt submit前のfailureはsubmission evidenceがfalseでprofileが許可したbounded new executionだけを使い、submit後のtimeout／disconnectは同一sessionのstatus／reattach／harvest／captureだけを許可して重複Prompt submitを禁止する。Oracleのprivate storage layoutへ依存する必要がある間はversioned capability boundaryを一つのinfra adapterへ隔離し、unsupported versionではfail closedする。 |
| E1-REQ-035 | Oracle authoring ZIPはuntrusted transient inputであり、Runtimeがcanonical三文書とonboarding companionのstrict UTF-8、LF、exact path／inventory、completeness、archive safetyを検証した後にだけ既存Candidate builderへ渡す。Candidate ID、source baseline、全payloadのMANIFEST role／SHA、CHECKSUMS、placeholder authority、Candidate ZIP identity、Review／Human decision、staging、承認前repository不変、承認後managed applyは引き続きRuntime authorityとする。 |
...
| E1-REQ-037 | すべてのIssue Planner／Semantic Revision runは、Runtimeがoperation identityから決定してPrompt本文とoutput expectationへ固定したexact relative pathに、new-member onboarding companion guideをexactly one生成する。guideはCandidate identity、MANIFEST role／SHA、CHECKSUMS、completeness validation、archive Review、Human decisionの対象であり、missing／wrong-path／duplicate／unmanifested guideではCandidateを生成しない。 |
| E1-REQ-038 | onboarding companionは正本三文書を理解するためのsubordinate explanatory artifactであり、第四のcanonical specificationではない。guideはauthority注記を持ち、Requirement／Design／Planと矛盾する場合は正本を優先し、その矛盾、誤ったimplementation status、Human authorityの弱体化、必須説明またはvalid PlantUMLの欠落をdefect-only Review findingとして扱う。 |
| E1-REQ-039 | archive ReviewはCandidate内のcompanionを三文書と同時にReviewする。git-bound Review／applyは`planning create`が生成した同一immutable Candidate ZIPをoperation evidenceとして必須受領し、Runtimeがそのvalidated MANIFESTのexactly-one `onboarding-companion` entryからpath／blob SHAを機械導出する。RuntimeはCandidate identity、source repository／branch／HEAD、companion path／SHAからcanonical `GitBoundOperationBindingV1` digestを生成し、canonical三文書のexact 3-path tupleとは別fieldでReviewed identity、Review JSON、Human decision、applyへcarryする。official Skillはcreate resultのexact Candidate pathをReview／applyへ引き継ぎ、operatorによるcompanion path／digest入力、directory scan／latest選択、arbitrary target、persistent registryを使用しない。missing／wrong／ambiguous Candidateまたはbinding mismatchはpre-mutationで拒否する。approved applyはcanonical三文書を変更せずreviewed blobsを検証し、Candidate内companionをmanaged Issue artifactとして同一transactionへ含め、Human decision前のrepository mutation 0とrollback／parityを維持する。 |
...
| E1-AC-017 | Initiative／Epic／Issue Candidateの3 package fixtureがScope別最小構成、source baseline、manifest／checksumsを満たす。Issue Candidateはcanonical三文書＋exactly-one onboarding companionを持ち、companionをcanonical roleへ分類せず、Initiative materialization contractを混入させない。 |
...
| E1-AC-027 | captured Oracle invocationで、compactなtask／role／exact branch／HEAD／fallback禁止／三つのcanonical文書＋onboarding companionのZIP inventory／subordinate authorityが`-p`等のPrompt本文に存在し、operation固有の詳細はprovider-owned operation resourcesから選択される。`--provided-context-path`の添付inventoryにはreference dataだけが存在し、operator-supplied planner／reviewer instruction、default-branch policy、text-frame output contractを命令authorityとして渡すfixtureは拒否される。 |
| E1-AC-028 | Planner／Semantic Revisionがexactly-one expected authoring ZIPを返すpositive fixtureからCandidateを生成できる。ZIPはcanonical三文書とexact expected onboarding companion pathだけを持つ。inline-only response、legacy text frame、ZIPなし、複数ZIP、wrong filename／root、missing／duplicate／wrong-path companion、missing／extra payload、unsafe entry、artifact metadataと実bytesのsize／SHA不一致はCandidate 0でfail closedする。 |
| E1-AC-029 | Oracle invocationはPATH-resolved executableへのdirect argvで一回だけsubmitされる。submit前のfailureはsubmission evidenceがfalseでprofileが許可したbounded new executionだけを使い、submit後のtimeout／disconnectはsame-session status／reattach／harvest／captureでartifactを回収する。sessionを安全に回収できないfixtureでは重複new submission、個人wrapper、API、default branchへのfallback 0でblockedとなる。 |
| E1-AC-030 | 検証済みauthoring ZIPから既存Runtimeがcanonical三文書＋onboarding companion＋control filesを持つimmutable Candidate ZIPを生成し、全payloadのMANIFEST／CHECKSUMS、Review identity、Human decision binding、三文書のCandidate-to-canonical parity、companionのmanaged-target parity、staging／rollback、validation／publicationの既存positive／negative fixtureがGreenである。Human decision前のtracked tree／index／HEAD mutationは0件である。 |
...
| E1-AC-032 | Issue Candidate fixtureのMANIFESTがcanonical三文書とonboarding companionを異なるroleで列挙し、CHECKSUMSが全payload bytesを覆う。missing／wrong-path／duplicate／unmanifested／checksum-mismatched companionはCandidate 0となる。 |
| E1-AC-033 | archive ReviewはCandidate内companionをexact identityへbindする。git-bound positive fixtureはcreate resultのsame CandidateをReview／applyへ渡し、Runtimeがexactly-one companion MANIFEST entryからpath／SHAと`GitBoundOperationBindingV1`を決定してcanonical 3-path tupleと分離し、fresh ReviewerをRuntime→provider-owned adapter→PATH Oracle→ChatGPT経由で起動し、Reviewerがexact current branchを独立確認したclosed JSONを同じ境界で返す。missing／wrong Candidate、zero／multiple companion role、Candidate SHA／source mismatch、Review後Candidate差替え、binding digest mismatch、directory auto-selection、manual companion path／digest入力はpre-Human mutation 0で拒否する。両modeはcanonical三文書との矛盾、誤ったcurrent status、Human／Oracle／exact-branch authorityの誤記をactual defectとして検出し、style preferenceだけではFAILせず、canonical三文書を第四のguide authorityで上書きしない。 |
| E1-AC-034 | representative onboarding companionがnew member向け必須sectionとsystem context、responsibility boundary、planning sequence、implementation roadmapをそれぞれ一件以上表す少なくとも四つのPlantUMLを持つ。responsibility boundaryとplanning sequenceはformal Reviewer pathをRuntime→provider-owned Oracle adapter→PATH-resolved Oracle→fresh ChatGPT Reviewerとして示し、Reviewerの独立したexact-current-branch確認とclosed Review JSONの同境界returnを省略しない。抽出した全diagramがPlantUML 1.2026.6 syntax checkをPASSする。approved archive applyではcompanionがmanaged Issue artifactとして三文書と同一transactionで配置され、fault injection時に三文書／guide／indexが一貫してrollbackされる。 |
...
- onboarding companionが正本より古い、矛盾する、または第四のcanonical specificationとして扱われる: exact source identity、distinct MANIFEST role、required authority note、defect-only consistency Review、managed artifact parityで停止する。
- git-bound Review／applyがcreate由来Candidateを失い、timestamped companionをdirectory scan、latest選択、operator入力、hidden stateで補おうとする: same immutable Candidate必須、exactly-one role導出、canonical operation binding digestでpre-mutation拒否する。
