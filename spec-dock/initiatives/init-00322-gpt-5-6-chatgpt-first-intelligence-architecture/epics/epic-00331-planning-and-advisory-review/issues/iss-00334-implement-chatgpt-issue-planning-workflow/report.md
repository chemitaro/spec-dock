...
| S08 | delegated | PATH Oracle direct adapter、capability preflight、single submit、same-session recovery、typed artifact snapshotのbounded implementation | dev-coder | `plan.md` S08と`artifacts/20260729t054034z-s08-chatgpt-implementation-work-packet.md`のexact allowlist | canonical三文書、S01〜S07実装履歴、Oracle boundary amendment、S08 work packet | S08-R01〜R17、focused infra／domain tests、ruff／format／mypy、runtime denylist、validate、diff check | supported Oracle capability／session artifactをboundedに取得不能、public contract変更、S09以降への越境、Oracle本体変更、personal wrapper／API／shell fallbackが必要 | delegated。worker evidence待ち |
...
| S08 packet | commit candidate | S08 ChatGPT work packetとReport execution-input evidence | packet review不要。ChatGPT ProがGitHub connectorでexact source HEAD照合済み | artifact SHA-256 `4808c3dcc40d34fe187e0a6a6b90b821d81278ea25378932f83c7d00e5a7fb6e` | commit／push／clean確認後にbounded dev-coderへ委任 |
