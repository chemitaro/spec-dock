...
| S08 | second bounded repair complete; fresh re-review pending | PATH Oracle direct adapter、capability preflight、single submit、same-session recovery、typed artifact snapshotのbounded implementation | same dev-coder | initial allowlist内のartifact helper／focused testsとopen P1-002／004だけ | canonical三文書、S01〜S07実装履歴、Oracle boundary amendment、S08 work packet、D-20260729-S08-COMPAT、EAL-20260729-S08-REVIEW-1／2 | exact 2 Red／Green、S08 focused、application compatibility、ruff／format／mypy、runtime denylist、validate、diff check | public contract変更、S09以降、Oracle本体変更、personal wrapper／API／shell fallback、旧frame API復活、P1 closureにS10／S11実装が必要 | exact 2 RedをGreen化。focused 127、application 66、CLI 2、親combined 195、ruff、mypy、validate、denylist、diff checkがpass。repair commit／pushと別fresh closure review待ち |
...
| S08 implementation | second repair complete; fresh re-review pending | S08 allowlisted direct Oracle runtime／domain／tests、temporary compatibility decision、Report evidence | repair commit `2f2b35f10d5480a328581fcf31c857d84f3a4937`のfresh closure reviewで3 closed／2 open／new 0、残2件をexact Red／Green | first repair commit／push済み、second repair commit pending | second repairをcommit／pushしnew immutable HEADで別fresh closure review |
...
- 次はsecond repair／Report／first closure FAIL artifactをcommit／pushし、new exact HEADで別fresh closure Reviewを行う。
- No material implementation decisions beyond the approved plan.
