...
| `cl-s03-path-input` | S03 | yes | implementation | S03/S04 atomic plan、v3 fresh plan review PASS、S03 brief（実装前） | pending implementation | pending | implementation evidenceはatomic resulting HEADで記録する |
| `tc-s03-001` | S03 | yes | implementation | v3 fresh plan review PASS、S03 brief（実装前） | pending implementation | pending | nested/hidden/symlink/FIFOのpath-only spyをatomic resulting HEADで実行する |
...
| S03-S04 | approved-atomic-implementation | implementation with per-step ChatGPT-Use briefs | dev-coder | S03/S04 union allowlist in plan.md、v2 atomic addendum、v3 fresh plan review | plan.md and current Issue scope | runtime、Review resource identity contract、e2e fixture、unit/transport tests、provider-sync projection as generated output | no bridge、generated pack、CLI/profile/recovery change、merge or close; both closures require same resulting HEAD | S03/S04 briefs、focused suite、legacy-removal search、provider parity、fresh code review、report closure | any out-of-allowlist production change or S03/S04 same-HEAD failure | begin atomic S03/S04 after S04 brief is recorded |
...
| S03-S04 | dev-coder | S03 path-only contract/caller とS04 direct transportをatomic change-setとして実装する準備が完了。v3 fresh plan review PASS後で、コード実装は未実施 | `artifacts/implementation-briefs/s03-input-path-model.md`, `artifacts/implementation-briefs/s03-s04-atomic-cutover-plan-clarification-v2.md` | plan-only verification; code tests not executed yet | PASS for plan gate; implementation pending | S04 dedicated briefを同じ最新HEADで生成し、両closureのsame-HEAD implementationへ進む |
...
| S03-S04 | implementation-readiness review | ChatGPT-Use Red Team | fresh | pass | no | promote atomic implementation preparation | v3 exact-HEAD review at `961a8b8370ed7e3e4cd162ebe15a55ef61101fe1` is PASS (P0=0/P1=0/P2=0/P3=0); S03/S04 code execution still requires per-step brief and same-HEAD closure |
