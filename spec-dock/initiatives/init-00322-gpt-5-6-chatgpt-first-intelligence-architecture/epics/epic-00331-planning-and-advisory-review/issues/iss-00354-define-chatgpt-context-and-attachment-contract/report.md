...
| EAL-010 | blocked_advisory | `/private/tmp/codex-agent-work/501/s03-brief-20260804/brief.md` | chatgpt-use-implementation-brief | S03 実装ブリーフを `artifacts/implementation-briefs/s03-input-path-model.md` に採用した。現行 allowlist（prompt/domain/tests）では `app/issue_planning.py` のbytes producer と `infra generated-pack consumer` への移行ができず、S03実装はplan amendmentまたはS03-S04 atomic cutover再承認が必要 | `artifacts/implementation-briefs/s03-input-path-model.md`, `report.md` | 仕様実装の実行前準備としてS03ブリーフ採用とscope-blockを記録 | advisory_blocked | `/private/tmp/codex-agent-work/501/s03-brief-20260804/brief.md` (SHA-256 `700b9c44cba1b66993cc30fd7fa1c52cefa6a79de59fd3b562ba2927cae682a8`) | issue orchestrator | ChatGPT-Use | no | briefをS03実装に先立ってrecordする。実装ready前のscope-blockを保持 |
| EAL-011 | partially_adopted | `/private/tmp/codex-agent-work/501/s03-s04-plan-20260804/brief-full.md` | chatgpt-use-plan-clarification | S03/S04 を一つの deployable change-set とし、S03 は application path-only contract/caller、S04 は direct repeated `--file` transport を担当する計画補正案を採用候補とした。両 closure は同一 resulting HEAD でのみ close する | `plan.md`, `artifacts/implementation-briefs/s03-s04-atomic-cutover-plan-clarification.md`, `report.md` | S03/S04 execution boundary、union allowlist、closure coupling | 現行コードの bytes producer と generated-pack consumer を既承認の path-only/direct transport 設計へ整合させるための最小補正。要件・設計、S05以降の責務は変更しない。fresh exact-HEAD plan review 前のため実装開始ゲートは未通過 | advisory_plan | `/private/tmp/codex-agent-work/501/s03-s04-plan-20260804/brief-full.md` (SHA-256 `3286db64b54a82c67237a637a9fad4bd4a9443b9f8e29590de97edccbd4ae2ea`), base HEAD `a2bc5e00cf7aefe049c234bfe0207f992077af8f` | issue orchestrator | ChatGPT-Use | yes | plan amendment を fresh exact-HEAD review で確認し、PASS 後に atomic S03/S04 実装を開始する |
...
- 計画補正ブリーフ: `artifacts/implementation-briefs/s03-s04-atomic-cutover-plan-clarification.md`
...
- Union allowlist: provider application `issue_planning_prompt.py`、`issue_planning.py`、infra `issue_planning_chatgpt.py`、対応する application/infra unit tests と transport integration test。domain、CLI、Oracle profile/recovery、artifact reader、projection、resource wording は read/run-only とする。
...
- reviewer gate: `adopted_pending_review`。この計画補正は ChatGPT-Use advisory として採用候補に記録したが、fresh exact-HEAD の defect-only plan review を通過するまで実装開始ゲートは blocked のままとする。
