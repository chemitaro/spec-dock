...
| S07 | implementation | provider-first docs, installed/dogfood projection parity, parent wording consistency | provider/projection SHA parity, cleanup analysis artifact, `spec-dock validate`, `git diff --check` | current S07 working tree; commit/push and fresh Red Team review are the next gate | green pending fresh review | runtime projection extras were removed; S07 scope excludes runtime code |
...
| S07 | projection/doc drift caused by mixed-generation update | S07 implementation brief and dogfooding observation | provider-first parity copy、S07対象外runtime差分の限定復元、untracked resources削除、parent wording check | `cl-s07-projection` | no | `artifacts/20260805t-projection-cleanup-analysis.md`、provider/dogfood parity |
...
| S07 | `cl-s07-projection` | provider/installed/dogfood parity and parent wording consistency | parity receipt and cleanup analysis recorded; fresh Red review pending | pending | close after S07 docs review on pushed HEAD |
...
| `cl-s07-projection` | S07 | yes | implementation | S07 brief, provider/projection parity, cleanup analysis, docs diff | parity and validate pass; fresh Red Team review pending | pending | close after pushed docs review |
...
| `cl-s06-blue-red`〜`cl-s13-closure` | S06〜S13 | execution-specific command output | not observed before implementation | pending | implementation will populate each row |
...
| S07 | active | docs/projection consistency with S07 ChatGPT-Use brief | doc-writer | provider skill/docs, installed/dogfood projection, parent Epic wording, S07 artifacts/report | plan.md and S07 brief | only provider/docs/projection/parent wording | no runtime code, unrelated docs, lifecycle state, merge or close | parity, validate, diff-check, fresh Red Team docs review | provider authority or projection parity ambiguous | commit/push then fresh Red Team review |
...
| S07 | doc-writer | provider-first docs/projectionを同期し、混入した別世代生成物を除去 | provider skill/docs、installed/dogfood projection、parent Epic、S07 artifacts/report | parity、validate、diff-checkを実行; fresh docs review pending | pending | commit/push後にfresh Red Teamへ送る |
...
| S07 | no delegation exception; docs-only projection cleanup is within approved S07 path | user request to implement and review; risk accepted: no | provider skill/docs, installed/dogfood projection, parent Epic wording, report/artifacts | provider-first sync, parity, cleanup analysis, docs review | no rollback needed; preserve provider source as authority | parity, validate, diff-check, fresh Red Team docs review | provider/projection mismatch or scope expansion | stop before S08, no merge or close |
...
| S07 | pending-review | current working tree | provider skill/docs、installed/dogfood projection、parent wording、cleanup analysis | commit/push後にexact HEADをfresh Red Teamへ送る | S07 docs/projection only | parity、validate、diff-check、fresh docs review | review PASSまでS07 closureを保留 |
...
|---|---|---|---|
| current branch HEAD | S05 implementation, test-only repair, v1/v2 Fresh Red Team evidence, and S06 handoff | reviewed exact HEAD is `ae58ef254e40ebb2fad4e64d8c22627fa312dae0`; S05 v2 output and SHA are recorded above; P0/P1=0でS05 closureを完了した。review artifact、implementation/repair brief、report evidenceのcanonical保存はproduction runtimeを変更しない | pass; S06 pending |
