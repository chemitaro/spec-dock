...
| S07 | implementation-repair | provider-first docs, installed/dogfood projection parity, parent wording consistency | v1 repair commit was pushed; Fresh Red v2 at exact HEAD `51ec4436...` FAIL（P0=0/P1=3/P2=0/P3=0）; bounded correction covers exact parity command/roots, historical eight-file scope, and pushed-state report wording | v1 review source `21a2c4c2bfb6e30a925e64f8bb9508687b128417`; v2 review source `51ec44361934991c0ba347eed7e5047c719ec122`; next repair HEAD will be supplied to fresh Red v3 | blocked; fresh Red v3 required | runtime projection extras were removed; S07 scope excludes runtime code |
| S08〜S13 | implementation preparation | inspect-only until each step's brief and execution gate | no execution evidence yet; executable step contracts remain in plan.md | docs inspection and runtime gate commands | pending | implementation evidence is collected per step |
...
| S07 | parity/scope/current-state evidence drift after v1 repair | Fresh Red `RT-354-S07-V2-001`〜`003` | complete parity command and distinct fresh-installed roots, five-direct/three-evidence historical scope reconciliation, and pushed-state ledger correctionをBlue修正中 | `cl-s07-projection` / `tc-s07-001` | no | EAL-046 FAIL、v2 review artifact、v2 Blue repair brief、cleanup/report addendum |
| S08〜S13 | no execution tests yet; closure risks are enumerated in plan.md | plan | no implementation response yet | `cl-s08-regression`〜`cl-s13-closure` | no | each step requires its own brief and evidence |
...
| S07 | `cl-s07-projection`, `tc-s07-001` | provider/installed/dogfood parity and parent wording consistency | v1 repair was committed/pushed; Fresh Red v2 at `51ec4436...` FAIL P1×3; bounded evidence correction is pending | pending | close only on fresh Red v3 PASS |
| S08〜S13 | `cl-s08-regression`、`cl-s09-profile`、`cl-s10-recovery`、`cl-s11-browser`、`cl-s12-artifact-reader`、`cl-s13-closure` | per-step behavior slice and gate in plan.md | no implementation observation yet; closure is pending execution | pending | implementation must populate each row per step |
...
| `cl-s07-projection` | S07 | yes | implementation-repair | S07 brief、Fresh Red v1/v2 FAIL、Blue repair briefs、provider/installed/dogfood recursive parity、validate、diff-check | four parity comparisons pass with distinct fresh-installed roots; v2 source `51ec4436...` has P1×3; command/scope/current-state correction pending | pending | close after fresh Red v3 PASS |
| `tc-s07-001` | S07 | yes | implementation-repair | provider-source preflight、fresh init、四組recursive parity、`parity_exclusions=[]`、historical eight-file scope audit | counts 7/7/37/37 and tree SHAs pass; v2 found incomplete exact command and scope receipt; pushed-state correction pending | pending | no S08 start before S07 PASS |
| `cl-s08-regression`〜`cl-s13-closure` | S08〜S13 | yes | inspect-only before implementation | runtime gate and per-step test command to be added at execution | not executed | pending | closure evidence is required during execution |
...
| `cl-s07-projection` / `tc-s07-001` | S07 | S07 brief、Fresh Red v1/v2 FAIL、Blue repair briefs、provider/installed/dogfood recursive parity、validate、diff-check、historical scope audit | four parity comparisons pass; v2 exact HEAD `51ec4436...` has P1×3; bounded evidence correction pending | pending | close only on fresh Red v3 PASS |
| `cl-s08-regression`〜`cl-s13-closure` | S08〜S13 | execution-specific command output | not observed before implementation | pending | implementation will populate each row |
...
| S07 | implementation-repair | S07 Fresh Red v1/v2 P1 repair with the existing Blue continuity | doc-writer | cleanup artifact, v1 Blue repair brief, report; v2 review/brief are evidence imports | plan.md, S07 brief, Red v1/v2 reviews, and Blue repair briefs | only three current correction files plus immutable evidence imports | no runtime code, unrelated docs, lifecycle state, merge or close | complete parity command/roots, historical eight-file scope audit, stale-state zero-match, validate, diff-check, fresh Red Team docs review | provider authority or projection parity ambiguous; any out-of-allowlist change | correct three files, commit/push, then fresh Red v3 review |
| S08-S13 | pending-next-step | implementation with per-step ChatGPT-Use brief | dev-coder | step-local allowed paths in plan.md | plan.md and current Issue scope | only the active step's allowed files | no execution before active step brief/review, no merge or close | per-step brief, tests, report closure | active step gate or capability ambiguity | begin S08 after S07 review PASS |
...
| S07 | doc-writer | v1 repair commitはpush済み。v2のP1三件（exact parity invocation/roots、historical eight-file scope、pushed-state ledger）だけを、v1 brief addendum、cleanup artifact、reportで修正する。v1/v2 Red outputはimmutable evidence importとして扱う | v1 Blue brief、cleanup artifact、report; v2 Blue brief/review evidence are imported separately | exact parity command exit 0; historical scope audit 5 direct + 3 evidence = 8 observed; v2 current-state evidence is pending correction; validate/diff-check required after edit | Fresh Red v1 FAIL (P1×4); Fresh Red v2 FAIL (P1×3); fresh Red v3 pending | v2 failure is recorded in EAL-046; commit/push after bounded correction, then fresh Red v3; PASS前はS07 closure/S08を開始しない |
| S08-S13 | dev-coder | not started; S07 closure completeが前提 | none | not executed | pending | active step briefとレビューが必要 |
...
| S07 | no delegation exception; bounded evidence repair remains within approved S07 docs/artifact path | user request to implement and review; risk accepted: no | v1 Blue repair brief, cleanup artifact, report; v2 brief/review files are immutable evidence imports | append-only brief/receipt/report correction, historical eight-file audit, stale-state check, fresh Red v3 handoff | no rollback needed; preserve provider source and all Red bytes as authority/evidence | exact parity command, four comparisons, historical scope audit, stale wording zero-match, validate, diff-check, fresh Red Team docs review | provider/projection mismatch, scope expansion, or v3 P1 finding | stop before S08, no merge or close |
...
| S07 | docs/projection repair review | ChatGPT-Use Red Team | fresh v1/v2; v3 not yet run | fail / repair pending | no | v1 exact-HEAD `21a2c4c2bfb6e30a925e64f8bb9508687b128417` FAIL (P0=0/P1=4); v1 repair pushed as `51ec4436...`; v2 exact-HEAD `51ec44361934991c0ba347eed7e5047c719ec122` FAIL (P0=0/P1=3); v2 evidence correction is bounded to three files and must be reviewed in a new v3 thread |
...
| S07 | repair-pending-review | v1 pushed repair HEAD `51ec44361934991c0ba347eed7e5047c719ec122` (v2 FAIL P1×3) | v1 brief addendum、cleanup artifact、report; v2 review/brief evidence import | v2 evidence correction is bounded to three current files; next commit/push then fresh Red v3 | S07 docs/evidence only; no runtime projection extras | exact parity command/roots、historical scope audit、stale-state zero-match、validate、diff-check | fresh Red v3 PASSまでS07 closureを保留 |
| S08-S13 | pending | not started | none | not applicable | step-local paths in plan.md; each step requires its own brief/review | not run | later steps await their own brief/review |
...
|---|---|---|---|---|
| docs / templates / README / workflow / skill / migration notes | yes | doc-writer | S07 v1 FAIL at `21a2c4c2...` and v2 FAIL at `51ec4436...` are retained; parity, eight-file scope, and current-state correction are under bounded repair | pending S07 fresh Red v3 |
...
| ChatGPT-Use Red Team | S07 provider Skill, parent Epic §6.3, operation-specific path option docs, projection parity evidence | v1 exact HEAD `21a2c4c2bfb6e30a925e64f8bb9508687b128417` FAIL (P0=0/P1=4); v1 repair pushed as `51ec4436...`; v2 exact HEAD `51ec44361934991c0ba347eed7e5047c719ec122` FAIL (P0=0/P1=3) for exact parity command/roots, historical eight-file scope, and pushed-state ledger | 2 fresh reviews; v3 must be a new thread against the next repair HEAD | FAIL / repair pending |

### 最終 spec review ゲート（Final Spec Review Gate）
| レビュアー（reviewer） | 範囲 | 指摘 / 修正（findings / fixes） | 再 review 回数（re-review count） | 結果（result） |
|---|---|---|---|---|
| spec-reviewer / ChatGPT-Use Red Team | requirement / design / plan / report / candidate identity alignment plus S03/S04 atomic plan amendment | v5 exact-head review at `079685b2...` is historical baseline PASS; S03/S04 plan amendment was separately reviewed at exact HEAD `961a8b8370ed7e3e4cd162ebe15a55ef61101fe1` and v3 is PASS with P0=0/P1=0; S07 docs review v1 at `21a2c4c2...` and v2 at `51ec4436...` are FAIL with P1×4 and P1×3 | v5 baseline plus S03/S04 plan v1/v2 FAIL repair cycle and v3 PASS; implementation code review is separate; S07 v3 repair review is pending | S03/S04 gate passed, but S07 docs gate is blocked until fresh v3 PASS |

### 最終 commit（Final Commit）
| 最終 report 台帳（final report ledger） | 最終 commit 範囲（final commit scope） | コミット後の外部証跡送付先（post-commit external evidence destination） | 結果（result） |
|---|---|---|---|
| current branch HEAD | S07 bounded evidence repair: v1 brief addendum, cleanup parity/scope receipt, and report current-state correction | v1 repair was committed/pushed as `51ec4436...`; Fresh Red v2 reviewed that exact HEAD and returned FAIL P1×3. v2 correction is currently uncommitted and must not claim PASS | commit/push the three-file correction, import v2 evidence bytes separately, then send the new exact HEAD to a fresh Red v3 thread | pending; S07 closure and final PR remain blocked |
