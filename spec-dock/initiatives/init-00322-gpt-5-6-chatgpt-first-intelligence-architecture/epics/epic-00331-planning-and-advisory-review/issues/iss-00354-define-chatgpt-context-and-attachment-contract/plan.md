...
| Next review identity | exact newly pushed branch tip supplied by the review prompt and verified through GitHub; default branch fallback is prohibited |
| Branch comparison | last reviewed tipとGitHub branch tipがidentical / ahead 0 / behind 0; subsequent repair commits require a fresh comparison |
...
- delegation contract: delegated role=`dev-coder`; input docs=S03 brief、S03/S04 atomic cutover clarification v2、`requirement.md`、`design.md`、application contracts、resource/e2e tests; allowed paths=`src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/issue_planning_prompt.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/issue_planning.py`, `src/spec_dock/assets/install_root/.agents/skills/spec-dock-issue-planning/resources/operations/review/attachments/instructions.md`（identity contract変更のみ）、`tests/unit/application/test_issue_planning_prompt.py`, `tests/unit/application/test_issue_planning.py`, `tests/integration/test_issue_planning_chatgpt_transport.py`, `tests/integration/test_issue_planning_e2e.py`; installed/dogfood resource projectionはprovider syncの生成出力として扱い手編集しない。forbidden changes=domain contract、symlink/FIFO traversal、path materialization、new content hashing、CLI、Oracle profile/recovery、output validators、上記以外のresource wording/inventory; acceptance criteria=`cl-s03-path-input` proves path-only application contract and callers on the same resulting HEAD as S04, with zero tree/content inspection; required verification=unit tests with read/rglob/stat/resolve spies、path order/object identity assertions、application regression、full-chain e2e; reviewer focus=`code-reviewer` checks path-only contract and caller immutability; stop conditions=compatibility bridge、generated identity file、new inspection rule、or any S04 consumer mismatch; output required=contract/caller/resource diff、spy result、tests、risks、and report Ledger Note.
...
- delegation contract: delegated role=`dev-coder`; input docs=S04 brief、S03/S04 atomic cutover clarification v2、`requirement.md`、`design.md`、Oracle transport adapter、CLI/integration tests; allowed paths=`src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/infra/issue_planning_chatgpt.py`, `tests/unit/infra/test_issue_planning_chatgpt.py`, `tests/integration/test_issue_planning_chatgpt_transport.py`, `tests/integration/test_issue_planning_e2e.py`, およびS03のunion allowlist（Review resource identity contractを含む）; installed/dogfood resource projectionはprovider syncの生成出力として扱い手編集しない。forbidden changes=generated pack、copy、ZIP、exclusion、retry/inline policy、Oracle wrapper、profile/stage/recovery、output validators、identity contract以外のresource wording/inventory; acceptance criteria=`cl-s04-direct-transport` proves exact repeated `--file` operands and no generated input pack on the same resulting HEAD as S03; required verification=argv equality test、no-tree/copy/archive/hash spy、transport CLI smoke、one-prompt/output regression、full-chain e2e; reviewer focus=`code-reviewer` checks direct Oracle boundary and one-shot semantics; stop conditions=direct capability unsupported、generated context appears、or S03 contract mismatch; output required=adapter/resource diff、argv receipt、tests、risks、and report Ledger Note.
...
- delegation contract: delegated role=`dev-coder`; input docs=S05 brief, `requirement.md`, `design.md`, command contracts, CLI tests; allowed paths=`src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/commands/issue_planning.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/issue_planning.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/issue_planning_prompt.py`, `tests/unit/commands/test_issue_planning.py`, `tests/unit/application/test_issue_planning.py`, `tests/unit/application/test_issue_planning_prompt.py`, `tests/cli_runtime/test_chatgpt_cli.py`, `tests/integration/test_issue_planning_chatgpt_transport.py`, `tests/integration/test_issue_planning_e2e.py`; `domain/issue_planning_contracts.py`, `infra/issue_planning_chatgpt.py`, and `cli/chatgpt_parser.py` are read-only inputs for this step; forbidden changes=Candidate publication format, closed JSON parser, unrelated commands, domain/infra/CLI contracts; acceptance criteria=`cl-s05-cli-cutover` proves planning/review/revision parser, repeatable original-path forwarding, and exact branch/repository/HEAD identity binding; required verification=command/application/prompt unit tests, CLI help contract, closed-identity test, transport regression, and the focused lifecycle integration test; reviewer focus=`code-reviewer` checks parser compatibility, original-path identity, and publication transaction; stop conditions=identity mismatch, old option still accepted, publication mutation, or a missing test allowlist for the hard-cutover help contract; output required=command/application/prompt diff, tests, identity receipt, risks, and report Ledger Note.
...
- delegation contract: delegated role=`dev-coder`; input docs=S10 brief, `requirement.md`, `design.md`, failure taxonomy, domain/application/CLI contracts; allowed paths=`src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/domain/issue_planning_contracts.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/issue_planning.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/infra/issue_planning_chatgpt.py`, `tests/unit/domain/`, `tests/unit/application/`, `tests/unit/infra/test_issue_planning_chatgpt.py`, `tests/unit/commands/test_issue_planning.py`, `tests/cli_runtime/test_chatgpt_cli.py`; forbidden changes=fallback/default mapping, post-submit new execution, unrelated recovery policy, artifact reader implementation reserved for S12; acceptance criteria=`cl-s10-recovery` proves exact internal-to-public status/reason mapping, infra recovery guard, and bounded call counts for false/unknown/submitted evidence; required verification=table-driven mapping tests, infra invocation call-count assertions, command/CLI serialization tests; reviewer focus=`code-reviewer` checks decision-to-infra boundary and `qa-reviewer` checks failure matrix; stop conditions=exact mapping or recovery budget is ambiguous; output required=decision/mapping diff, invocation receipt, matrix result, tests, risks, and report Ledger Note.
...
- step gate / report destination: exact mapping、infra guard、または recovery budget が崩れたら S11 を開始しない。変更した domain/application/infra/CLI の各 closure とテスト結果を `report.md` に記録する。
...
- delegation contract: delegated role=`dev-coder`; input docs=S11 brief, `requirement.md`, `design.md`, prompt corpus, browser smoke matrix, sanitized receipts; allowed paths=`src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/`, `tests/unit/application/`, and the opt-in smoke script; forbidden changes=private credentials, browser profile, raw transcript, unrelated Oracle wrapper; acceptance criteria=`cl-s11-browser` proves logical model request, observed label/verified flag, direct/inline attachment, response, and artifact fields share one attempt lineage; required verification=receipt schema assertions and opt-in browser smoke with sanitized output; reviewer focus=`qa-reviewer` checks external evidence and `code-reviewer` checks prompt/attachment wiring; stop conditions=logical model/observed-label verification or required attachment delivery is unverified; the S11 brief's `GPT-5.6 Luna / Max` authoring setting is recorded separately and is not a product runtime success condition; output required=sanitized receipt, test result, model evidence status, risks, and report Ledger Note.
...
- step gate / report destination: logical model、observed label、verification flag、または attachment delivery が未確定なら未検証として停止し、S11 brief のモデル設定とは分離して `report.md` に記録する。
...
- delegation contract: delegated role=`dev-coder`; input docs=S12 brief, `requirement.md`, `design.md`, artifact fixtures, reader contracts, and the S10 mapping closure; allowed paths=`src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/infra/issue_planning_oracle_artifact.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/infra/issue_planning_chatgpt.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/domain/issue_planning_contracts.py`, `src/spec_dock/assets/spec_dock/scripts/spec_dock_runtime/application/issue_planning.py`, `tests/unit/infra/`, `tests/unit/domain/`, `tests/unit/application/`, `tests/integration/`; forbidden changes=generic hardcoded harvest/capture outside the selected profile, unsafe extraction, automatic regeneration, new public mapping (consume S10 mapping); acceptance criteria=`cl-s12-artifact-reader` proves response/download/capture separation, exact profile capture, strict ZIP validation, generic adapter cleanup, and S10 `REQ-030` mapping reuse; required verification=0.16.1/0.17.0 fixture matrix, invocation spy, mapping reuse assertion, and integration output; reviewer focus=`code-reviewer` checks extraction safety/version binding and `qa-reviewer` checks fixture coverage; stop conditions=missing/ambiguous artifact, path escape, or mapping duplication; output required=reader/adapter diff, fixture results, mapping receipt, integration output, risks, and report Ledger Note.
