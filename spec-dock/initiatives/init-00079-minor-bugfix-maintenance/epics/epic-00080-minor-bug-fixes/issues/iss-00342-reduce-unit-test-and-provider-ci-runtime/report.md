...
Materialな判断がない場合はno-decisionを明示する。本IssueではD-001〜D-003がcurrent decision ledgerである。
...
| EAL-006 | `adopted` | system-architect delegated evidence | `design.md` / `plan.md` | native pytest marker、partial-safe classifier、7 required-fast nodes、専用global completeness verifier、2-file workflow、Make command facade、F/H partition、3 paired final measurement batch、rollbackを採用する | `artifacts/20260728t041725z-delegated-draft-test-lane-architecture.md`; SHA-256 `4ecf5a906b12a1a5469cff65086421eaae6138caafd3a148be77fc51090f0792`; `specialist_status=usable`; `diff_guard_result=passed`; collection `C=2696/F=661/H=2035/U=0`; focused smoke / parity `7 passed in 2.02s`; design-R3 fresh pass | implementation-planner evidenceとcanonical planへ引き渡す |
