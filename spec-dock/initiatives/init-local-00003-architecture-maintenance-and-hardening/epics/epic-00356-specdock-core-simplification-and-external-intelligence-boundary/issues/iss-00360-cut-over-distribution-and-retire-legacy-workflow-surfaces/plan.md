...
- Full-regression boundary: `uv run pytest --run-full-regression`は必ず実行して結果を記録する。ただしProvider Full Regressionは`.github/workflows/provider-full-regression.yml`の`main` push / `workflow_dispatch`で独立して実行され、PR fast gateの代替ではない。固定点と現行で同じcommandを実行し、現行で失敗する各test pathのfailure behaviorが固定点でもfailureであり、Issue 360のchanged hunk / required selection外であるfailureは、全体回帰の不具合を隠さず`approved-no-op`の残課題としてcount、path、owner、follow-upをreportへ記録できる（固定点と現行のfailure総数が一致することは要求しない）。固定点でpassして現行でfailとなったpath、failure behaviorが変化したpath、Issue 360のchanged pathでのfailure、またはrequired selectionのfailureはS95 blockerとしてowner stepへ戻す。この扱いはfull-regression workflowの実行・可視化・post-merge修正を免除しない。
