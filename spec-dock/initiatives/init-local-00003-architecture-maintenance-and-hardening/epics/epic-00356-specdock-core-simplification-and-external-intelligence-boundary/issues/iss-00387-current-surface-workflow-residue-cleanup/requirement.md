...
- current RequirementまたはDesignのhunk、または§15の既存rowにある`状態`・`Evidence reference`のcell value以外のcurrent Plan hunkを検出した場合は`SPECIFICATION_CONTRACT`としてstageせず、C60保持物をexact cleanupしてから、original implementation baseline由来の実装前treeでclean fixed SHA Strict再reviewとC00-01新規実行へ返す。
