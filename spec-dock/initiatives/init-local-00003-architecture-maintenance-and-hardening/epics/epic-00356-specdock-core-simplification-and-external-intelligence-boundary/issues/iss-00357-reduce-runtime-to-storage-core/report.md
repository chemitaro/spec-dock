...
- S99主要full-regressionは`188 passed, 35 skipped`、`make lint`、`validate`（nodes=221）、`doctor`（findings=0）、`git diff --check`がpassした。追加の全テスト実行でIssue 334のArtifact実体に依存する2テストを特定し、ユーザー判断によりIssue 357のRuntime契約外テストとして削除した。削除後の通常`uv run pytest`は`1285 passed, 2185 skipped`である。
- 削除対象は`tests/unit/domain/test_issue_planning_candidate.py`の`test_s10_current_v4_guide_satisfies_completeness_contract`と`test_current_managed_guide_matches_current_milestone_state`、および不要になった`zipfile` importだけである。合成payloadを使う同ファイルのvalidator契約テスト53件とIssue 357のRuntime実装は保持した。
- S99のbounded fresh content reviewは、code-reviewerがP0/P1/P2なしでpassし、spec-reviewer / qa-reviewerも実装・仕様・テスト内容のP0/P1/P2を検出しなかった。Artifact依存テスト削除後は、ユーザー判断・新しいverification・report ledgerを対象に三者再レビューを行う。
...
| `DEC-357-S99-001` | superseded | baseline failure disposition | S99の`tests/unit`と通常`uv run pytest`で、Issue 334の既存Artifact欠落を参照する同一2テストが失敗した | Issue 357で外部Artifactを補完する / `covered-existing + explicit out-of-scope`としてIssue 334 ownerへ追跡する | 初期観測として記録。ユーザーの後続判断により`DEC-357-S99-002`へ置換し、これら2テストを削除する | 初期観測 `1285 passed, 572 skipped, 2 failed`、`1285 passed, 2185 skipped, 2 failed` | Artifact復旧をIssue 334へ残す方針は、削除によりIssue 357のCI blockerではなくなった |
| `DEC-357-S99-002` | resolved | obsolete external-artifact test removal | Issue 334の管理Artifact実体を読む2テストがStorage Core / Issue 357 Runtime契約と無関係で、Artifact欠落だけでProvider CIを失敗させていた | 外部ArtifactをIssue 357へ追加する / fixture化する / 不要な外部Artifact依存テストを削除する | adopted by explicit user decision: `test_s10_current_v4_guide_satisfies_completeness_contract`、`test_current_managed_guide_matches_current_milestone_state`、不要な`zipfile` importを削除。`validate_onboarding_companion`の合成payload契約テスト53件は保持し、Runtime sourceは変更しない | 削除後 `tests/unit/domain/test_issue_planning_candidate.py`: `53 passed`; `uv run pytest`: `1285 passed, 2185 skipped`; `make lint`; `validate nodes=221`; `doctor findings=0` | Issue 334のArtifact復旧はIssue 357のCI前提から切り離す |
...
| `tests/cli_runtime/test_authoring.py` | removed通常`authoring` CLI casesと360 authoring-pack direct / compatibility coverageが同居 | Modify in S01 / S10 preserving 360 cases; S10 removes obsolete normal preflight method / removed source-path assertions only | 357 |
...
| `tests/unit/infra/test_runtime_resolver_workbench_opacity.py` | removed `assurance_store` / `delegated_authoring` casesとretained `delete_node` workbench-opacity safety caseが同居 | Modify in S10: remove only removed-module cases / helpers and preserve `test_delete_fallback_scan_prunes_workbench_and_preserves_near_name` | 357 |
| `tests/unit/infra/test_oracle_selector_removal.py` | removed `commands/authoring.py` pathと360 authoring-pack / skill / docs selector surfacesが同居 | Modify in S10: remove only the deleted command path and preserve every 360 selector surface / assertion | 357 |
...
| `application/contracts.py::{ShowAssuranceRequest,ClassifyAssuranceRequest,VerifyAssuranceRequest,ComposeAssuranceRequest,AssuranceOperation,AssuranceResultStatus,ComposeArtifactSelection,AssuranceTargetView,ComposeArtifactView,AssuranceResult,WorkflowStatusRequest,WorkflowNextRequest,RunbookProjectionResult,WorkflowResult,ArtifactImportRequest,ArtifactImportResult,ArtifactImportError,UseCases.import_artifact,UseCases.show_assurance,UseCases.classify_assurance,UseCases.verify_assurance,UseCases.compose_assurance,UseCases.workflow_status,UseCases.workflow_next,UseCases.repo_root,UseCases.specdock_dir}` | removed command / delegated-authoring / assurance application contract。Runbook / WorkflowStateを含むworkflow result typeを明示 | Delete candidate after S01 removes corresponding adapters and bootstrap wiring; dependent Assurance result family adopted by `DEC-357-S10-009` | 357 |
...
| `tests/unit/infra/test_runbook_store.py` | `application.workflow`、`domain.runbook`、`domain.workflow_state`、`infra.runbook_store`とremoved workflow contractだけを直接coverし、retained / 360 consumerなし | Delete candidate with workflow / runbook Runtime in S10; E00 omission adopted by `DEC-357-S10-008` | 357 |
| `tests/unit/domain/test_authority.py` | `domain.authority`の旧promotion / grant gateだけを直接coverし、S02〜S09後のretained / 360 consumerなし | Delete candidate with `domain.authority` in S10; E00 omission adopted by `DEC-357-S10-008` | 357 |
| `tests/unit/domain/test_artifact_composer.py` | removed `domain.artifact_composer` / `domain.assurance`だけを直接coverし、retained / 360 consumerなし | Delete candidate with artifact composer / assurance Runtime in S10; E00 omission adopted by `DEC-357-S10-008` | 357 |
...
| `{application,domain,infra,presentation}/authoring_pack/**` | `application/issue_planning.py`がpreflight、authority boundary、ZIP contractをstatic import | Handoff keep; S10 may remove only the two deleted normal-authoring paths from `domain/authoring_pack/source_manifest.py::DEFAULT_SOURCE_PATHS` per `DEC-357-S10-010` | 360 |
...
| `tc-s99-001` | S99 | red-required + delta | 初回fresh三者reviewでS99未実施、stale active-set test、dirty worktreeをP1検出。追加の全テストでIssue 334 Artifact依存テスト2件を特定し、ユーザー判断により削除した。修正後にtargeted / ordinary、主要Issue-local regression、lint、validate、doctor、diff-checkを再実行し、DEC-357-S03-011 / DEC-357-S99-002をledgerへ確定 | §7 Verification sequence、obsolete external-artifact test removal、fresh三者再レビュー、全テストGreen | 通常`uv run pytest`は`1285 passed, 2185 skipped`、`tests/unit/domain/test_issue_planning_candidate.py`は`53 passed`、Issue-local commandは`188 passed, 35 skipped`。`make lint`、validate `nodes=221`、doctor `findings=0`、diff-checkもpass。fresh三者再レビューとfollow-up commit後のPR観測へ進む |
...
| `uv run pytest tests/unit` | `1285 passed, 572 skipped` |
| `uv run pytest` | `1285 passed, 2185 skipped` |
...
Issue 334のArtifact実体に依存する2テストは、ユーザー判断によりIssue 357のテスト対象から削除した。`validate_onboarding_companion`の合成payload契約テストは保持しており、Storage Core Runtimeの挙動・API・正本R/D/Pは変更していない。S99 content gateはfresh三者passであり、follow-up commit後にclean statusとrefs不変を確認する。
...
| M99 / S99 | pre-M99 content review pass、M99 commit済み。Artifact依存テスト削除後のfollow-up reviewを実施中 | `docs(iss-00357): 最終実装証跡を確定` / final report ledger + two contract-aligned tests | committed | `51470580ddc245c5446898ac887ec4db1f6765dd` | pass: M99 commit直後のclean status / refs invariant。follow-up test cleanupのcommit / PR再観測を実施 |

## 残余リスクと停止条件

- Runtime削除対象にretained consumerが見つかった場合はE00で停止する。
- `domain.authority`とprofile Artifact経路はS02 / S04 / S05 / S08 / S09でconsumerを外し、S10 exhaustive audit / fresh review後に削除した。
...
Artifact依存テスト削除とreport ledger更新を通常のgit add / git commitで確定し、commit後にbranch、parent、refs、git diff --check、clean statusを検証してPR #362へ再プッシュする。PR観測とmergeは別workflowであり、Issue finishは実行しない。
