...
- managed regular file / symlink / obsolete prune / generated version を既存 descriptor-bound kernel で適用し、action checkpoint、staging lease、exact pre/postcondition から partial failure を forward recovery できるようにした。
...
- [x] `uv run pytest tests/unit/infra/test_managed_distribution.py` — 113 passed
...
- [x] `uv run pytest` — 1049 passed、1044 policy-skipped（Issue 368 の最終修正前に実行。最終 candidate で再実行する）
- [x] `./spec-dock/scripts/spec-dock validate` — `nodes=227`
- [x] `git diff --check` — 成功
- [ ] ChatGPT Final Quality Gate Strict — commit/push 後の exact candidate SHA で実施予定
