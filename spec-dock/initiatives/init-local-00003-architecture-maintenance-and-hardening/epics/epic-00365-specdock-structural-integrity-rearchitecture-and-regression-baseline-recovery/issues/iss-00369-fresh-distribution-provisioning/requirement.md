...
fresh target に対する `spec-dock init` を、`iss-00368` で成立した unified reconciliation engine へ移す。利用者は衝突のない repository に Desired Managed Assets を完全に provision でき、不明な parent/target state がある場合は write 前に保護される。fresh-only scaffold mutation engine、plan 外 callback、特殊な publish path は残さない。
...
| ID | 要件 |
|---|---|
| I369-R01 | `spec-dock init [path]` の command/flag/exit semantics を維持し、fresh intent を D1 の service/contract/action/kernel/journal/result へ追加する。 |
...
1. fresh `init` がD1のservice、action grammar、kernel、journal、ProcessResultを使用する。
...
8. update/init-forceがfresh-only workbench seedをbackfillしないcurrent semanticsを維持する。
...
- `--force` behaviorは本Issueのfresh intentに混ぜない。recognized `init --force`はD1のownerである。
