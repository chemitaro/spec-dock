...
# iss-00288 Epic から Issue 候補を作る候補専用パックをドッグフードする — 実装計画
...
この Issue の local `authorized_profile` は `.assurance.json` / `assurance classify` を権威とし、ChatGPT 推奨や Epic 側の推奨グレードでは上書きしません。この Issue は candidate-only dogfood に限定し、profile-specific canonical template を出さないため、追加 obligation は standard とします。execution-ready と扱うには、fresh `spec-reviewer` result を `report.md` に残します。
