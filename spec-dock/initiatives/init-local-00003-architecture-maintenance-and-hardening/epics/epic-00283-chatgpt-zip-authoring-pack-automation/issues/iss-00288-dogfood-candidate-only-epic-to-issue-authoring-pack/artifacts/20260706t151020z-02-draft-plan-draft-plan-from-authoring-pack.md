...
intended_targets:
  - "spec-dock/initiatives/init-local-00003-architecture-maintenance-and-hardening/epics/epic-00283-chatgpt-zip-authoring-pack-automation/issues/iss-00288-dogfood-candidate-only-epic-to-issue-authoring-pack/plan.md"
...
- created_by_role: `chatgpt-use`
- scope_id: `iss-00288`
- intended_target: `spec-dock/initiatives/init-local-00003-architecture-maintenance-and-hardening/epics/epic-00283-chatgpt-zip-authoring-pack-automation/issues/iss-00288-dogfood-candidate-only-epic-to-issue-authoring-pack/plan.md`
- adoption_status: `unreviewed`
- reflected_to: `[]`
- diff_guard_result: `passed: issue-local draft artifact only; no canonical target reflected`
- report_evidence_destination: `spec-dock/initiatives/init-local-00003-architecture-maintenance-and-hardening/epics/epic-00283-chatgpt-zip-authoring-pack-automation/report.md#EAL-008`
...
# iss-00288 Epic から Issue 候補を作る候補専用パックをドッグフードする — ドラフト実装計画
...
3. Epic レベルの ZIP から複数 Issue 候補を作り、プロファイル推奨だけを返すことを検証する。
4. 成果物を candidate-only ZIP fixture、candidate validation report、Issue 比較 summary として作る。
...
- candidate-only ZIP fixture、candidate validation report、Issue 比較 summary が存在する。
- 親 trace E-RQ-011 / E-AC-007, E-AC-011 を説明できる。
...
- その後、次 Issue `iss-00289` を `./spec-dock/scripts/spec-dock issue start iss-00289` で開始する。
