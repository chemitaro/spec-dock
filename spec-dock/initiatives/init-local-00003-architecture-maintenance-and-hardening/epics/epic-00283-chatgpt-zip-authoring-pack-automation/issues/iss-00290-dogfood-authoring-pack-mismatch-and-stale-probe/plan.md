...
# iss-00290 不一致・期限切れパックをブロックできるか検証する — 実装計画
...
この Issue の local `authorized_profile` は `.assurance.json` / `assurance classify` を権威とし、ChatGPT 推奨や Epic 側の推奨グレードでは上書きしません。ただし、この Issue は stale / source mismatch / profile mismatch / unsafe claim を fail-closed にブロックする negative probe を担うため、strict 相当の追加 obligation を持ちます。execution-ready と扱うには、manual fallback evidence、failure-mode record、fresh `spec-reviewer` result を `report.md` に残します。
