...
# iss-00292 ドッグフード指標とランタイム昇格基準を評価する — 実装計画
...
この Issue の local `authorized_profile` は `.assurance.json` / `assurance classify` を権威とし、ChatGPT 推奨や Epic 側の推奨グレードでは上書きしません。この Issue は指標評価と昇格判断材料の作成に留め、ランタイム昇格自体を決定しないため、追加 obligation は standard とします。execution-ready と扱うには、fresh `spec-reviewer` result を `report.md` に残します。
