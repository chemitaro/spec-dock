...
consumer workflowとprovider CIのownerを分離し、build-once artifactとLinux canonical / macOS deltaの一意なpolicy contractを完成させる。
...
3. Product interviewでmaterial policyを受理する。
4. accepted decision、artifact receipt contract、change / removal handoffを作る。
5. Epic docsへ反映する。
...
3. seed / reusable projection / excludedのownership案とlifecycle authorityを比較する。
4. wheel / sdist / Linux / macOSのtrigger、build invocation、digest、retention、reuse案を比較する。
5. Product ownerへ最推奨と代替を提示し、各policyを明示受理する。
6. accepted ADRまたは追補へownershipとreceipt schemaを記録する。
7. install/update Issueが変更するshipped assetとCI Issueが変更するprovider workflowを別receiptにする。
8. Epic Requirement / Design / Planをactual decisionへ更新する。
...
- workflow ownership、trigger、artifact identity、build count、retention、lane ownerが受理されている。
