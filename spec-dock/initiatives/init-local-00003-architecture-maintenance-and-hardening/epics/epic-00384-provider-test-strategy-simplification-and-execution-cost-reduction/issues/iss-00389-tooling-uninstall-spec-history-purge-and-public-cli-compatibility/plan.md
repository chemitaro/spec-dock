...
normal uninstall、任意の独立purge、public compatibilityを相互に分離し、後続実装がdelete authorityを一意に判断できるaccepted contractを完成させる。

## 順序・依存

1. 現行CLI / intent / result / retry surfaceをinventory化する。
2. remove / deprecate / independent purgeの選択肢を比較する。
3. destructive authority、confirmation、sunset、result semanticsをProduct interviewで受理する。
...
3. 各選択肢のdata-loss risk、compatibility cost、sunset、migration guidanceを比較する。
4. Product ownerへ最推奨と代替を提示し、material choiceを個別に受理する。
5. accepted ADRまたは追補へcommand contractとdelete authorityを記録する。
6. 後続Issueの削除対象option、intent、route、journal、test、docsをreceipt化する。
7. Epic canonical docsをactual decisionへ更新する。
