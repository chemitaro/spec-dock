...
matrixは`cutover_path = split | combined`を追加inputに持つ。splitではP1を公開し、combinedではP1全cellを`unpublished / N/A`としてP0からP2へ直接移る。D1は両path-specific digestを受理し、C4がconsumer graphとbridge acceptanceからactual pathを選択して該当digestへ束縛する。
