...
| U1 | 小さいinstallerとruntime起動へ切替、旧lifecycle撤去 | 置換・データ不変・失敗再実行テスト、runtime smoke | 実装済み・統合検証中 |
| U2 | provider認証の残consumerと旧テスト/policy/CIを撤去 | 全collection、通常pytest、lint、package smoke | 実装済み・統合検証中 |
