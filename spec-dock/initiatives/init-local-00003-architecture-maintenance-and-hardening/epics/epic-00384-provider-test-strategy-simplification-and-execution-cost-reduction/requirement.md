...
- R8: 更新中は関連コマンドを停止して利用する。複数ディレクトリの原子的交換、クラッシュ後の自動再開、ロールバック、並行更新は保証しない。失敗時は外部installerで同じ更新を最初からやり直す。途中失敗を成功と報告しない。
...
3. 不完全な配布元と危険な配置先は変更前に拒否する。途中コピー失敗は非ゼロになり、同じ更新の再実行で正常配置へ戻る。
