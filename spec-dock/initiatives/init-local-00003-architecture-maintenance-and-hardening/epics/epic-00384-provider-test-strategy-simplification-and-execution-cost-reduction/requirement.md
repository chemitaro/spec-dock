...
12. **Stability predicates.** Rolling twenty windowのflakesはexactly zero、retriesまたはrerunsはexactly zeroである。Retry/rerunを行ったexecutionを除外、置換またはclean passへ昇格しない。
13. **Fail-closed rejection.** Fingerprint drift、missed seeded fault、flake、retry、rerun、incomplete five-run evidence、incomplete rolling-twenty window、identity mismatchまたはmissing raw evidenceはqualification rejectionである。
