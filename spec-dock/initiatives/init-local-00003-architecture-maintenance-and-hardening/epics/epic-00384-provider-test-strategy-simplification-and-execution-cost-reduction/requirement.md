...
#392/#395とPR #403の修正は既存履歴。#395のURL認証情報をエラーへ漏らさない性質など、残る機能の回帰は守る。Issue状態を勝手に変更しない。主要移行は#396（PR #404）で実施した。2026-09-20のユーザー指示により、移行後の残存実装・テスト・文書の最終cleanupは子Issue #405で実施する。R1〜R10とaccepted ADRのProduct契約は変更しない。通常の仕様管理・GitHub操作・worktree機能の廃止や、利用者データの移行・削除は今回の目的に含めない。
