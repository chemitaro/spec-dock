...
updateの途中失敗では管理6ディレクトリに混在状態が残り得る。利用者データは触らない。エラーを返し、外部installerのupdateをもう一度行う。初回initのスキャフォールド配置が失敗した場合は、利用者が不完全な `spec-dock/` を別名または別の場所に移して保全し、initを再実行する。既存の利用者データがある場合も自動削除しない。updateやforce-initは `.gitignore` などの初期設定を修復しない。途中状態の記録、旧版バックアップ、cleanup token、候補認証、自動復旧は作らない。通常のコマンドと更新の同時実行は運用上禁止する。
