...
Epic #384は `iss-00387 / #387 Current Surface Workflow Residue Cleanup` を外部前提として扱う。両scopeの所有権は次のとおり固定する。
...
4. `iss-00387` nodeがmainへmergeされた後にSpecDockの正式dependencyを追加する。それまではGitHub #387をtextual handoff authorityとする。
...
- historical specs / docs / fixtures、現行2 skills、consumer `ci.yml`、issue-start checkout helperをcleanup対象へ拡張しない
...
- external prerequisite / handoff: `iss-00387 / #387`。local nodeがmainへmergeされるまではtextual handoffとし、merge後に正式dependencyを登録する
