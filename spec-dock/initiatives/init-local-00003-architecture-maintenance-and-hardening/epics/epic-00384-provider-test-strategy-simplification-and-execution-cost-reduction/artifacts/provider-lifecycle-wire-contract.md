...
Parser: UTF-8、regular file、link count1、max4096 bytes、duplicate/unknown/missing key rejection。Writer mode0644、atomic replace only。Public-record staging objectは最終public mode0644で作成・fsyncしてからnative no-replace/exchangeで公開し、rename/exchangeがmodeを保存することを検証する。Publicへmode0600で公開してからchmodする遷移は禁止する。
