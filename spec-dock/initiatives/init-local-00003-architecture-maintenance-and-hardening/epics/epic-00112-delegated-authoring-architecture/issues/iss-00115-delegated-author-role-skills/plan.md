...
# iss-00115 Delegated Author Role Skills — 実装計画（Execution Contract）
...
- step 依存 summary:
  - S01: Provider source update; depends on iss-00113, iss-00114; unblocks S02.
...
- behavior goal:
  - Provider-side source of truth contains the `Role skills` contract.
- design 参照:
...
- `tc-s01-001` inspect-only: provider contract exists
  ...
  - 操作: inspect diff.
  - 期待結果: `spec-dock-system-architect と spec-dock-implementation-planner の provider-first role skills を追加する。` is represented without contradicting parent Epic.
