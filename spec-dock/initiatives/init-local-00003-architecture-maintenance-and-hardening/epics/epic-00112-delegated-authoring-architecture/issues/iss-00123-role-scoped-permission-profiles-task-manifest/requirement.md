...
# iss-00123 Role Scoped Permission Profiles and Task Manifest Probes — 要件定義（何を、なぜ行うか）

## 目的
- epic-00112 の v1 amendment を実装可能な追加 Issue として具体化し、role-scoped Permission Profiles, task manifests, resolved path allowlists, probes, and fail-closed fallback policy.
...
- この Issue が閉じる親 Epic 項目: E-RQ-008, E-RQ-010, E-RQ-012 / E-AC-008, E-AC-011。
- 情報源:
...
- 対象外:
  - OS sandbox as sole security boundary
  - Copilot/.github/agents support
  - write grant for implementation agents
...
- AC-001:
  ...
  - 操作: Task manifest records resolved canonical target, input revision/hash, allowed paths, forbidden paths, probe commands, and fallback.
...
- AC-002:
  - アクター: spec-dock maintainer / delegated authoring orchestrator
  - 前提: epic-00112 v1 amendment is the approved parent scope.
  - 操作: Positive write probe succeeds only for allowed artifact/evidence paths.
  - 期待結果: The behavior is observable in provider artifacts, dogfooding evidence, or report gate evidence.
  - 観測点: issue report, managed assets, reviewer verdict, and targeted tests.
- AC-003:
  - アクター: spec-dock maintainer / delegated authoring orchestrator
  - 前提: epic-00112 v1 amendment is the approved parent scope.
  - 操作: Negative write probe fails for forbidden implementation/config paths.
  - 期待結果: The behavior is observable in provider artifacts, dogfooding evidence, or report gate evidence.
  - 観測点: issue report, managed assets, reviewer verdict, and targeted tests.
- AC-004:
  - アクター: spec-dock maintainer / delegated authoring orchestrator
  - 前提: epic-00112 v1 amendment is the approved parent scope.
  - 操作: Fail-open, unreproducible, or Desktop/CLI divergent probe disables write-scoped delegation.
  - 期待結果: The behavior is observable in provider artifacts, dogfooding evidence, or report gate evidence.
  - 観測点: issue report, managed assets, reviewer verdict, and targeted tests.

## 例外・エッジケース
- EC-001:
  - 条件: Symlink/active path resolution drift invalidates the manifest.
...
- EC-002:
  - 条件: Forbidden path write success is fail-open and blocks enablement.
  - 期待: fail closed, record evidence, and keep v0 workflow available.
  - 観測点: report decision ledger / reviewer finding / targeted test or inspection evidence.
- EC-003:
  - 条件: Probe unavailable produces documented fallback, not verified operation.
  - 期待: fail closed, record evidence, and keep v0 workflow available.
  - 観測点: report decision ledger / reviewer finding / targeted test or inspection evidence.
