...
# iss-00125 Authority Aware Delegated Authoring Dogfooding Pilot — 要件定義（何を、なぜ行うか）

## 目的
- epic-00112 の v1 amendment を実装可能な追加 Issue として具体化し、dogfood v1 delegated authoring with proposed canonical drafts, promotion gates, lifecycle blocks, permission fallback, validate/sync, and reviewer evidence.
...
- この Issue が閉じる親 Epic 項目: E-RQ-010, E-RQ-011, E-RQ-012 / E-AC-010, E-AC-011, E-AC-012 plus operational evidence for E-AC-001..E-AC-009。
- 情報源:
...
- 対象外:
  - new provider implementation inside the pilot
  - overstating host invocation as verified
  - rewriting v0 issue reports/plans
...
- AC-001:
  ...
  - 操作: Pilot records prerequisite closure for iss-00120..iss-00124 or explicit fallback.
...
- AC-002:
  - アクター: spec-dock maintainer / delegated authoring orchestrator
  - 前提: epic-00112 v1 amendment is the approved parent scope.
  - 操作: Actual delegated design and plan drafts remain authority proposed and are promoted only by main orchestrator plus final reviewer.
  - 期待結果: The behavior is observable in provider artifacts, dogfooding evidence, or report gate evidence.
  - 観測点: issue report, managed assets, reviewer verdict, and targeted tests.
- AC-003:
  - アクター: spec-dock maintainer / delegated authoring orchestrator
  - 前提: epic-00112 v1 amendment is the approved parent scope.
  - 操作: Lifecycle/context-pack block or fallback is observed for proposed artifacts.
  - 期待結果: The behavior is observable in provider artifacts, dogfooding evidence, or report gate evidence.
  - 観測点: issue report, managed assets, reviewer verdict, and targeted tests.
- AC-004:
  - アクター: spec-dock maintainer / delegated authoring orchestrator
  - 前提: epic-00112 v1 amendment is the approved parent scope.
  - 操作: Completed Issue 001..006/#113..#118 plans and reports remain unmodified; v1 closure is additive.
  - 期待結果: The behavior is observable in provider artifacts, dogfooding evidence, or report gate evidence.
  - 観測点: issue report, managed assets, reviewer verdict, and targeted tests.

## 例外・エッジケース
- EC-001:
  - 条件: Permission Profile unavailable results in fallback evidence, not verified operation.
...
- EC-002:
  - 条件: Provider defect creates follow-up/amendment instead of being silently fixed in pilot.
  - 期待: fail closed, record evidence, and keep v0 workflow available.
  - 観測点: report decision ledger / reviewer finding / targeted test or inspection evidence.
- EC-003:
  - 条件: Host invocation unverified is recorded without over-claiming.
  - 期待: fail closed, record evidence, and keep v0 workflow available.
  - 観測点: report decision ledger / reviewer finding / targeted test or inspection evidence.
