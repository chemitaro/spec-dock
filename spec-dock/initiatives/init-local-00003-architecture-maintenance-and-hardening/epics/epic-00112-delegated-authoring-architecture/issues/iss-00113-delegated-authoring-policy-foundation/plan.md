...
# iss-00113 Delegated Authoring Policy Foundation — 実装計画（Execution Contract）
...
- step 依存 summary:
  - S01: Provider source update; depends on none; unblocks S02.
...
|---|---|---|---|---|---|---|---|---|---|---|
| tc-001 | S01 | provider contract | acceptance | AC-001 | Provider source contains the Policy foundation contract. | target provider files | missing shipped contract | yes | inspect-only | report step closure |
...
- behavior goal:
  - Provider-side source of truth contains the `Policy foundation` contract.
- design 参照:
...
- `tc-s01-001` inspect-only: provider contract exists
  ...
  - 操作: inspect diff.
  - 期待結果: `workflow_spec_authoring.md に canonical ownership / draft-only delegation / consent / forbidden actions を追加する。` is represented without contradicting parent Epic.
