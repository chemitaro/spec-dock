...
# iss-00113 Delegated Authoring Policy Foundation — 要件定義（WHAT / WHY）

## 目的
- workflow_spec_authoring.md に canonical ownership / draft-only delegation / consent / forbidden actions を追加する。
- 親 Epic の `E-RQ-001, E-RQ-002, E-RQ-003` / `E-AC-001, E-AC-002` を、この Issue の変更範囲で閉じられる状態にする。

## 背景・現状
- 現状の挙動:
  - Epic `epic-00112` は delegated authoring を draft-only evidence として導入する方針を固定した。
  - この Issue の対象領域 `Policy foundation` は、まだ shipped provider assets と dogfooding consumer workspace に実装されていない。
- 現状の課題:
...
- AC-001:
  ...
  - 操作: 対象成果物を確認する。
  - 期待結果: `workflow_spec_authoring.md に canonical ownership / draft-only delegation / consent / forbidden actions を追加する。` が provider-side source of truth に反映される。
