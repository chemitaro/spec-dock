...
# iss-00116 Delegated Authoring Phase Gates — 要件定義（WHAT / WHY）

## 目的
- phase_design / phase_plan / phase_plan_epic / phase_plan_issue に delegated authoring gates と reviewer criteria を組み込む。
- 親 Epic の `E-RQ-003, E-RQ-008, E-RQ-011` / `E-AC-004/E-AC-005 gate readiness, E-AC-006, E-AC-009` を、この Issue の変更範囲で閉じられる状態にする。

## 背景・現状
- 現状の挙動:
  - Epic `epic-00112` は delegated authoring を draft-only evidence として導入する方針を固定した。
  - この Issue の対象領域 `Phase gates` は、まだ shipped provider assets と dogfooding consumer workspace に実装されていない。
- 現状の課題:
...
- AC-001:
  ...
  - 操作: 対象成果物を確認する。
  - 期待結果: `phase_design / phase_plan / phase_plan_epic / phase_plan_issue に delegated authoring gates と reviewer criteria を組み込む。` が provider-side source of truth に反映される。
