...
# iss-00116 Delegated Authoring Phase Gates — 実装計画（Execution Contract）
...
|---|---|---|---|---|---|---|---|---|---|---|
| tc-001 | S01 | provider contract | acceptance | AC-001 | Provider source contains the Phase gates contract. | target provider files | missing shipped contract | yes | inspect-only | report step closure |
...
- behavior goal:
  - Provider-side source of truth contains the `Phase gates` contract.
- design 参照:
...
- `tc-s01-001` inspect-only: provider contract exists
  ...
  - 操作: inspect diff.
  - 期待結果: `phase_design / phase_plan / phase_plan_epic / phase_plan_issue に delegated authoring gates と reviewer criteria を組み込む。` is represented without contradicting parent Epic.
