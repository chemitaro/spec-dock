...
# iss-00162 Align Skill Docs Template Context Surfaces — 要件定義
...
- 必須:
  ...
  - 必要最小限の first cleanup を、後続 issue の前提を壊さない範囲で provider source に反映する。
...
- 対象外:
  ...
  - `spec-dock-clarification` skill-owned grill workflow の本体 rewrite。
  - Hub / leaf routing の本体 rewrite。
  - Workflow docs 全体の本体 alignment。
  - Templates 全体の本体 rewrite。
...
- 常に行う:
  ...
  - 後続 issue へ渡す項目は、対象 issue、理由、blocking / non-blocking、推奨処理を trace する。
- 判断が必要:
...
- This issue creates the cross-surface inventory and priority cleanup baseline; it must not absorb the implementation scope assigned to `iss-00163`, `iss-00164`, `iss-00165`, or `iss-00166`.

## 前提

- `iss-00159` は T1 specimen として完了済みである。
- この issue は T2 inventory / consistency lane であり、後続 T2/T3/T4 issue の土台になる。
- Draft-only upstream evidence は canonical authority ではなく、正式 requirement への採用と fresh reviewer pass を経て使う。
...
- AC-005:
  - アクター: 後続 issue の planner
  - 前提: `iss-00163` / `iss-00164` / `iss-00165` / `iss-00166` の planning を開始する
  - 操作: この issue の inventory / trace matrix を読む
  - 期待結果: 後続 issue が自分の担当 surface と非担当 surface を判断できる
  - 観測点: discussion / report evidence
...
- EC-003:
  - 条件: template wording が compliance authority と読める
  - 期待: この issue では contradiction を inventory し、global template rewrite は `iss-00166` へ渡す
  - 観測点: trace matrix
...
- TERM-004:
  - Trace matrix: contradiction / cleanup target / owner issue / evidence path / blocking status を一覧化する表。
