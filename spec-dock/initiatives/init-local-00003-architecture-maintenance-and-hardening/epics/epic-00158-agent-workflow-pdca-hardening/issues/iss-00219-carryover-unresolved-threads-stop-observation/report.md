...
| `uv run pytest tests/unit/infra/test_init_update.py -k "issue_214 or issue_187_s204 or issue_187_s430 or issue_218_s01_review_collector_no_findings or issue_218_s03_wait_no_findings or issue_219_s01"` | 40 passed, 435 deselected | Issue214 progress regression and no-findings reviewed-commit mismatch regression subset |
| `uv run pytest tests/unit/infra/test_init_update.py -k "issue_219 or issue_218_s01_review_collector_no_findings or issue_218_s03_wait_no_findings or issue_187_s420 or issue_187_s430 or issue_187_s204 or github-pr-observation or pr_observation"` | 113 passed, 362 deselected | broad PR observation regression subset after Issue214 regression and reviewed-commit mismatch fixes |
| `uv run pytest tests/unit/infra/test_init_update.py::TestInitUpdate::test_issue_71_checked_in_dogfooding_agent_tooling_parity_matches_install_root_assets` | 1 passed | provider/mirror agent-tooling parity after progress/no-findings fixes |
| `./spec-dock/scripts/spec-dock validate` | passed | `spec-dock: ok (validate) nodes=137` after progress/no-findings fixes |
...
| 4 | `#221` | `d3295cb3957499aef12ae65dd59421f919589718` | `wait_pr_observation.sh` out: `/private/tmp/issue219-pr221-wait-20` | fixed `@codex review` trigger posted; CI 4 checks passed; final JSON returned `human_gate`, reason `current_selected_unresolved_thread`, recommended action `address_review_feedback`; selected review comment `3446284485` reported fallback-pass documentation drift | actionable feedback を本 issue の修正対象として採用し、skill documentation を実装済み昇格条件に同期 |
| 5 | `#221` | `48e17c40f15d5427bf79d45ad9e11427f895e93d` | `wait_pr_observation.sh` out: `/private/tmp/issue219-pr221-wait-21` | fixed `@codex review` trigger posted; CI 4 checks passed; final JSON returned `human_gate`, reason `current_selected_unresolved_thread`, recommended action `address_review_feedback`; selected review comment `3446321476` reported no-findings Reviewed commit mismatch risk; progress log also showed Issue214 regression `review=observing` during no-signal/actionable wait | actionable feedback and observed progress regression を本 issue の修正対象として採用 |
...
| PR review follow-up: fallback-pass documentation drift | `SKILL.md` operator-facing contract | 実装は `fallback_pass_candidate.promotes_top_level_status=true` の統合済み no-findings path を `passed` / `merge_prepared` に昇格できるが、skill 説明は非昇格 signal のままだった | generic fallback は human gate のまま、strict no-findings contract と blockers integration 後のみ top-level result へ昇格可能と明記 | `git diff --check`, `spec-dock validate`, provider/mirror parity 1 passed |
| PR review follow-up: no-findings Reviewed commit mismatch must not promote | `pr_review_snapshot.py` strict no-findings recognition | 遅延した別head向け no-findings issue comment が current trigger window に入ると、`Reviewed commit` の存在だけで最新headを review 済み扱いにできる | multi-line no-findings form は `Reviewed commit` metadata を抽出し、`expected_head_sha` と prefix match する場合だけ strict no-findings と認識する | new regression `test_issue_219_s01_review_collector_no_findings_reviewed_commit_mismatch_does_not_promote`; focused subset 40 passed; broad subset 113 passed |
| PR monitoring regression: Issue214 `review=observing` reappeared | `pr_observation_wait.py` progress line | Issue214/PR #216 で廃止した observer-side `review=observing` が Issue218 merge 後に復活し、wait 中の target state を誤読させる | wait 中かつ completion signal / actionable feedback がない `none` / `pending` / `unknown` / legacy `approved` / `passed` は `review=pending_signal` と表示し、unresolved / changes_requested / commented は target state と counters を維持する | focused subset 40 passed; broad subset 113 passed; `rg` confirmed `render_review = "observing"` absent |
...
| PR feedback fixes after final ledger | fallback-pass documentation sync, no-findings reviewed-commit match, Issue214 progress regression fix, provider/mirror sync, regression tests, report evidence | PR #221 push and PR observation manual test rerun | pending commit |
