...
| D-PR223-001 | resolved | implementation | Codex PR review / dev-coder / orchestrator | `statusCheckRollup` を廃止した後も merge-preparer predicate は visible merge conflict evidence を要求するため、代替 mergeability signal が必要になった | A: merge conflict evidence を持たない; B: 常に human gate にする; C: PR metadata `mergeable` を許可された PR metadata として収集し、`CONFLICTING` は merge conflict、`UNKNOWN` / missing は conservative human gate とする | C を採用。`mergeable=CONFLICTING` は `resolve_merge_conflict`、`UNKNOWN` / missing は `pr_mergeability_unknown` として non-pass human gate にする | `mergeable` は PR metadata であり、GitHub Checks API / status rollup / commit statuses / `gh pr checks` ではない。merge conflict evidence を捨てず、かつ forbidden CI surface を再導入しない最小変更 | applied | PR #223 Codex review comments 3447329768; `pr_observation_snapshot.py`; `test_issue_222_pr_observation_snapshot_conflicting_mergeability_is_human_gate`; `test_issue_222_pr_observation_wait_unknown_mergeability_is_human_gate`; code-reviewer `019ee66a-a5db-7b71-92b1-80e970cbeff5` pass | `mergeable=UNKNOWN` は GitHub の一時状態になり得るため、pass ではなく human gate として扱う |
