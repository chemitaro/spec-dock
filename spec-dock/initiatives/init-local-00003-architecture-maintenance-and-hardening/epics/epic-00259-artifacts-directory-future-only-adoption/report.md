...
- 現在地（何が完了し、何が未完か）:
  ...
  - Executable Issues `iss-00262` through `iss-00267` have completed implementation commits.
  - Issue `iss-00268` dogfooding evidence has been executed in the active dogfooding workspace: `blank`, `research`, and `draft-requirement` artifacts were created under issue-local `artifacts/`; canonical `requirement.md` stayed unchanged; legacy `discussions/` regular files and `rules.md` symlink stayed unchanged; `validate` and `sync` passed.
  - Issue `iss-00268` final commit / review is no longer pending; final dogfood evidence was committed as `1ed85b04` (`docs(dogfood): artifacts移行の最終証跡を記録`).
  - Epic-wide P1 repair integration has completed for role configs / docs, artifact ADR mirror eligibility, delegated artifact authority guard normalization, dogfooding runtime mirror drift, and `new artifact` / `artifacts/` test expectations.
  - Latest spec re-review P1 repair evidence: `.codex/AGENTS.md` provider/mirror capability summary no longer says discussion docs can be created; it now limits creation/import to initiatives / epics / issues and working artifacts under `artifacts/` via `new artifact`, with discussions legacy / preservation only.
  - Latest code re-review P1 repair evidence: delegated artifact authority guard now normalizes quoted YAML-ish scalars, so `authority: "accepted"` with `mirror_eligible: true` and editable delegated draft state is rejected before sync mirror; provider / dogfooding mirror files match.
  - Repair validation passed: `git diff --check`; `./spec-dock/scripts/spec-dock validate` (`spec-dock: ok (validate) nodes=171`); focused previous failure test passed before the full run; delegated authoring focused suite `56 passed, 29 skipped in 30.95s`; full gate rerun `1689 passed, 74 skipped in 938.84s`. One previous full run had one timeout / flaky failure in `test_issue_182_s03_wait_decision...`, but the focused rerun passed and the latest full rerun passed.
- 次のマイルストーン:
  - Rerun Epic-wide spec / code reviews after the latest P1 repairs; QA re-review already passed before these latest repairs and the full gate has passed again. Create the single Epic-level PR only after spec / code re-review returns pass.
- ブロッカー:
  - None currently identified; remaining work is Epic-wide spec / code re-review and PR delivery after those re-reviews pass.
...
| EAL-009 | adopted | issue execution / dogfood command output | report.md / Epic acceptance status | Executed Issue `iss-00268` evidence proves old-node on-demand `artifacts/` creation and legacy `discussions/` non-migration in the active dogfooding workspace. The final dogfood evidence is committed and adopted for Epic progress. | `iss-00268` active issue report; `artifacts/20260701t145916z-dogfood-blank-artifact.md`; `artifacts/20260701t145919z-research-dogfood-research-artifact.md`; `artifacts/20260701t145944z-draft-requirement-dogfood-draft-requirement.md`; `validate` output `nodes=171`; `sync` output with active unchanged and no tracked projection diff; commit `1ed85b04` | Completed for Issue `iss-00268`; latest repair state is tracked by EAL-010. |
| EAL-010 | adopted | Epic-wide review findings / repair validation | report.md / E-AC-010 / quality gate status | Epic-wide P1 repairs are integrated. Spec P1 repair: `.codex/AGENTS.md` provider/mirror capability summary no longer says discussion docs can be created; it now says initiatives / epics / issues can be created/imported, working artifacts are created under `artifacts/` via `new artifact`, and discussions are legacy / preservation only. Code P1 repair: delegated artifact authority guard normalizes quoted YAML-ish scalars, so `authority: "accepted"` with `mirror_eligible: true` and editable delegated draft state is rejected before sync mirror; provider / dogfooding mirror files match. Earlier QA P1 repair and full-gate evidence remain valid, and QA re-review already passed before these latest two P1 repairs. | repaired `.codex/AGENTS.md` provider/mirror capability summary; delegated artifact authority guard and provider/dogfooding mirror files; `git diff --check`; `./spec-dock/scripts/spec-dock validate` -> `spec-dock: ok (validate) nodes=171`; focused previous failure test passed; `uv run pytest tests/unit/domain/test_delegated_authoring.py tests/cli_runtime/test_delegated_authoring.py -q` -> `56 passed, 29 skipped in 30.95s`; latest full gate `uv run pytest tests/unit tests/cli_runtime -q` -> `1689 passed, 74 skipped in 938.84s`; one previous full run had one timeout / flaky failure in `test_issue_182_s03_wait_decision...`, but focused rerun passed and the latest full rerun passed. | Rerun Epic-wide spec / code reviewers and keep the single Epic-level PR pending until those re-reviews return pass. |
...
- `iss-00262`: Done; commit `f48d505b` (`feat(artifacts): artifactテンプレート基盤を追加`).
- `iss-00263`: Done; commit `6d532548` (`feat(artifacts)!: new artifact コマンドへ移行`).
- `iss-00264`: Done; commit `c9e29244` (`feat(scaffold)!: 新規ノードをartifacts既定へ切り替え`).
- `iss-00265`: Done; commit `1e9031e9` (`feat(runtime): artifacts対応の検証と同期投影を追加`).
- `iss-00266`: Done; commit `3f7ee2f9` (`fix(delegated-authoring): 委任成果物の境界をartifactsへ移す`).
- `iss-00267`: Done; commit `510d1945` (`docs(workflow): artifacts向けのガイダンスに揃える`).
- `iss-00268`: Done; commit `1ed85b04` (`docs(dogfood): artifacts移行の最終証跡を記録`).
- Epic-level PR / Release: pending until Epic-wide spec / code re-reviews pass; one PR remains pending after spec / code pass.

## 受け入れ条件（E-AC）の達成状況 (必須)
- E-AC-001 `new artifact` command creation: Pass. Evidence: Issue `iss-00263` command migration plus Issue `iss-00268` blank smoke created `artifacts/20260701t145916z-dogfood-blank-artifact.md` without `blank` in the filename.
- E-AC-002 typed artifact filename: Pass. Evidence: Issue `iss-00262` artifact template / filename foundation plus Issue `iss-00268` research smoke created `artifacts/20260701t145919z-research-dogfood-research-artifact.md`.
- E-AC-003 `new doc` removal: Pass. Evidence: Issue `iss-00263` breaking command migration commit `6d532548`.
- E-AC-004 artifact catalog coverage: Pass. Evidence: Issue `iss-00262` and `iss-00263` commits; Issue `iss-00268` exercised `blank`, `research`, and `draft-requirement`.
- E-AC-005 ADR source collection: Pass. Evidence: Issue `iss-00265` validation / sync projection commit `1e9031e9`.
- E-AC-006 draft artifacts safety: Pass. Evidence: Issue `iss-00263` draft command migration plus Issue `iss-00268` `draft-requirement` smoke created `artifacts/20260701t145944z-draft-requirement-dogfood-draft-requirement.md`; canonical `requirement.md` diff remained empty.
- E-AC-007 validation layouts: Pass. Evidence: Issue `iss-00265` commit `1e9031e9`; Issue `iss-00268` `./spec-dock/scripts/spec-dock validate` passed with `nodes=171`.
- E-AC-008 scaffold and docs alignment: Pass. Evidence: Issue `iss-00264` scaffold commit `c9e29244` and Issue `iss-00267` docs / workflow commit `510d1945`.
- E-AC-009 old-node on-demand artifact setup: Pass. Evidence: Issue `iss-00268` baseline had no `artifacts/` directory and had legacy `discussions/rules.md`; `new artifact` commands created `artifacts/` while `discussions/` regular files remained empty and `rules.md` symlink target remained unchanged.
- E-AC-010 dogfooding evidence: Pass for dogfood and repair verification. Evidence: Issue `iss-00268` blank / research / draft-requirement smokes passed; `validate` and `sync` passed; final Issue evidence was committed as `1ed85b04`; latest repair validation passed with `git diff --check`, `./spec-dock/scripts/spec-dock validate` (`nodes=171`), focused previous failure rerun, delegated authoring focused suite `56 passed, 29 skipped in 30.95s`, and full gate rerun `1689 passed, 74 skipped in 938.84s`. One previous full run had one timeout / flaky failure in `test_issue_182_s03_wait_decision...`, but focused rerun passed and the latest full rerun passed. Epic-wide spec / code re-review remains pending until reviewers rerun; QA re-review already passed before the latest two P1 repairs and the full gate has passed again.
- Epic-wide quality gates and one Epic-level PR: Partially passed. Dogfood and repair verification passed; QA re-review passed before the latest spec / code P1 repairs; Epic-wide spec / code re-review is pending; one PR remains pending after spec / code pass.

## ロールアウト結果（必要なら） (任意)
- 段階公開の状況:
  - 該当なし。Epic delivery remains planned as one PR after Epic-wide spec / code re-reviews pass.
- 監視値（エラー率/レイテンシなど）:
  - 該当なし。Runtime command evidence is covered by tests and dogfooding validation, not production rollout telemetry.
- 障害/アラート:
  - 該当なし。

## フォローアップ（別Issue化） (必須)
- 現時点で新規 follow-up Issue は起票していない。
- Remaining non-follow-up work: Epic-wide spec / code re-review and the single Epic-level PR after those re-reviews pass.
