...
| D-010 | resolved | implementation | live PR #173 Codex review follow-up after `c17016d7` | PR #173 の再 observation 後、CI は passed したが Codex review が issue comment `updated_at`、`mergeStateStatus=UNKNOWN`、combined status aggregate、snapshot 後 head race、`reviewDecision=CHANGES_REQUESTED` の5件を P2 として指摘した。 | A: 実装修正だけ行い仕様は据え置く; B: EC-010 として requirement/design/plan に境界条件を追加し、実装・テストを同期する | B を採用する。issue comment は `updated_at` も trigger-window signal に含める。`mergeStateStatus=UNKNOWN` は blocking ではなく wait/poll の `pending` とする。commit status aggregate `state` は individual statuses が切り詰められる場合の backstop として使う。snapshot collection 後に PR head を再検証し、変更があれば `stale_head` とする。GraphQL `reviewDecision=CHANGES_REQUESTED` は blocking review status とする。trigger command issue comment 自体は、編集後 `updated_at` が trigger 後でも current-window body / raw artifact から除外する。 | いずれも long-running PR observation の false merge-prepared / false human_gate / stale snapshot acceptance / trigger command echo を防ぐ境界であり、正本仕様・実装・回帰テストを同期する必要があるため。 | promoted_to_requirement_design_plan | `requirement.md`; `design.md`; `plan.md`; `tests/unit/infra/test_init_update.py`; PR #173 comments `3370344393`, `3370344395`, `3370344396`, `3370344397`, `3370344398`; initial fresh spec-reviewer found `plan.md` EC-010 closure omission; initial fresh code-reviewer found implicit snapshot head drift, order-dependent stale assertion, and edited trigger command body/artifact leakage; all fixed; latest focused `pr_review or pr_observation` = `71 passed`; provider/mirror parity and `git diff --check HEAD~1..HEAD` pass | fresh spec-reviewer / push 後に PR #173 を再 observation する |
| D-011 | resolved | implementation | live PR #173 Codex review follow-up after `ebb8b327` | PR #173 の再 observation 後、CI `provider-tests` failed run `27110207713` と Codex review comments `3370406683`, `3370406685`, `3370406687` が出た。 | A: flaky test だけ直し、thread/timestamp 指摘は後続 issue に送る; B: EC-011 として requirement/design/plan に境界条件を追加し、実装・テストを同期する | B を採用する。semantic-change wait test に CI 余裕を追加する。REST review comment の `thread_id` と GraphQL thread node id を突合し、GraphQL `comments(last: 100)` に古い comment id がない resolved/outdated thread でも state を反映する。review / inline review comment の trigger 同秒 timestamp は current trigger-window signal として扱わない。 | いずれも PR observation の false blocker / false current feedback / CI flake を防ぐ境界であり、implementation-only にすると正本契約と reviewer gate が乖離するため。 | promoted_to_requirement_design_plan | `requirement.md`; `design.md`; `plan.md`; `tests/unit/infra/test_init_update.py`; PR #173 comments `3370406683`, `3370406685`, `3370406687`; CI run `27110207713`; dev-coder focused `pr_observation` = `57 passed`; targeted CI-failed test = `1 passed`; provider/mirror parity and `git diff --check` pass | fresh spec-reviewer / code-reviewer / push 後に PR #173 を再 observation する |
| D-012 | resolved | implementation | live PR #173 observation / Codex review follow-up after `2037847d` | PR #173 の再 observation 中、GitHub checks は pass / `mergeStateStatus=CLEAN` だったが snapshot は `commit_statuses.total=0` かつ `aggregate_state=pending` により `ci=pending` に固定された。Codex review comments `3370458188`, `3370458189` も出た。 | A: observation timeout と review comments を follow-up issue に送る; B: EC-012 として empty aggregate、same-second review collapse、bootstrap config symlink migration を正本へ追加し、この issue で修正する | B を採用する。空の commit statuses aggregate `pending` / `failure` / `error` は backstop にしない。実 `statuses[]` が1件以上ある aggregate failure/error backstop は維持する。同秒同 reviewer review collapse は numeric/API-order tie-breaker を使う。bootstrap-only `.codex/config.toml` migration は symlink を辿らない。 | 3点とも current PR observation の false wait / false review gate / repo 外 config 書き換えに直結するため、merge-prepared quality gate 前にこの issue 内で閉じる必要がある。 | promoted_to_requirement_design_plan | `requirement.md`; `design.md`; `plan.md`; snapshot for PR #173 head `2037847d4149d0b653389a4499e449cdc702ab8a`; PR #173 comments `3370458188`, `3370458189`; `gh pr checks` all pass; `mergeStateStatus=CLEAN`; fresh code-reviewer first fail then re-review pass | fresh spec-reviewer / push 後に PR #173 を再 observation する |
...
- S04 の初回 code-reviewer findings は、GraphQL reviewThreads pagination、trigger command comment の status 判定除外、`out-only` raw body artifact 保持の regression tests と実装修正で対応した。fresh code-reviewer re-review は pass。
...
- S04 の re-review P1 は、raw artifact を stdout body と同じ trigger window boundary に揃え、unknown trigger では `raw/review_bodies.json` を empty array にする regression test で修正した。fresh code-reviewer re-review は pass。
...
- 追加指摘は dev-coder が targeted config migration、wait semantic fingerprint、top-level human gate preservation、actual command detection、focused regression で修正した。初回 fresh reviewers は broad migration、review semantic stability reset gap、raw review body が wait semantic fingerprint に混入し得る gap を検出した。dev-coder の追加修正で exact migration、review semantic fields、raw body を除外する sanitized signal projection、semantic-change / raw-body-only regression を追加した。fresh spec-reviewer / code-reviewer は final re-run で pass した。push 後の再 observation は未実施。
- PR #173 の `c17016d7` 再 observation 後、CI は passed したが Codex review が issue comment `updated_at`、`mergeStateStatus=UNKNOWN`、combined status aggregate、snapshot 後 head revalidation、`CHANGES_REQUESTED` reviewDecision の5件を指摘した。
- 追加指摘は requirement/design/plan に EC-010 として反映し、dev-coder が provider/mirror scripts と focused regressions で修正した。初回 fresh spec-reviewer は `plan.md` の EC-010 closure 漏れを P1 として検出し、初回 fresh code-reviewer は `--head-sha` 未指定 snapshot の post-collection head drift を P2 として検出した。plan closure と implicit collection head revalidation を追加修正し、main-session focused `pr_observation` は `57 passed`、provider/mirror parity と `git diff --check` は pass。fresh spec-reviewer / code-reviewer re-run は pass。
...
- S05 の初回 code-reviewer P1 は、snapshot poll を remaining deadline で bound し、`subprocess.TimeoutExpired` を `snapshot_poll_timeout` limitation 付きの parseable final JSON に変換する regression test で修正した。fresh code-reviewer re-review は pass。
...
- S05 の re-review P1 は、`zero_checks_s03_non_success` を grace-managed limitation として扱い、`poll < zero_check_grace_polls` では wait、到達時に machine-readable non-success とする regression test で修正した。fresh code-reviewer re-review は pass。
...
- S05 の quiet 起点 P1 は、`semantic_fingerprint(payload)` 計算後の post-observation monotonic time で `latest_change_monotonic` を更新する regression test で修正した。fresh code-reviewer re-review は pass。
...
| main orchestrator / reviewers | PR #173 fourth review feedback follow-up | added | live observation for `906f30b0d94305dfa38bdd58186364ee09753949` found CI `provider-tests` failed in run `27107713057` and Codex review comments `3370261493`, `3370261494`, `3370261495`. dev-coder fixed semantic fingerprint stability, bootstrap-only config migration, human gate preservation, and trigger command detection; dev-coder focused `pr_observation` = `49 passed`, focused new regressions = `3 passed`, slow timeout regression = `1 passed`; main-session `pr_observation` = `49 passed`; full `uv run pytest tests/unit/infra/test_init_update.py -q` = `271 passed`; syntax / provider-mirror parity / `git diff --check` pass. Fresh spec-reviewer then found P2 broad bootstrap config migration and later P2 raw body inclusion in wait semantic fingerprint; fresh code-reviewer found a review semantic stability reset gap. dev-coder narrowed migration to exact known stale guidance, exposed `review.fingerprint`, included sanitized review semantic fields in wait stability, and added preservation / semantic-change / raw-body-only regressions. Latest main-session focused `pr_observation` = `51 passed`; full `uv run pytest tests/unit/infra/test_init_update.py -q` = `273 passed`; `bash -n` provider/mirror wait scripts pass; provider/mirror wait script `diff -u` pass; `git diff --check` pass. Final fresh spec-reviewer pass; final fresh code-reviewer focused 5 tests = `5 passed`, `pr_observation` = `51 passed`. Later follow-up rows record subsequent push / PR re-observation cycles. | pass |
| main orchestrator / reviewers | PR #173 fifth review feedback follow-up | added | live observation for `c17016d7c4bc37690c9f5b90e46625fc52b76e0d` found CI passed / review unresolved / `human_gate`; Codex review comments `3370344393`, `3370344395`, `3370344396`, `3370344397`, `3370344398` addressed. dev-coder fixed issue-comment `updated_at`, `mergeStateStatus=UNKNOWN` polling, combined status aggregate backstop, post-collection head revalidation, and `CHANGES_REQUESTED` reviewDecision handling. Initial fresh spec-reviewer P1 (`plan.md` EC-010 closure omission), initial fresh code-reviewer P2 (`--head-sha` omitted snapshot head drift), later fresh code-reviewer P1 order-dependent stale assertion, and P2 edited trigger command issue comment body/artifact leakage were fixed. latest focused `uv run pytest tests/unit/infra/test_init_update.py -k 'pr_review or pr_observation' -q` = `71 passed`; review script `bash -n` provider/mirror pass; provider/mirror `diff -u` pass; `git diff --check HEAD~1..HEAD` pass. Final fresh code-reviewer found only this report gate wording drift, and the ledger was corrected. Later follow-up rows record subsequent fresh spec-review / push / PR re-observation cycles. | pass |
| main orchestrator / reviewers | PR #173 sixth review / CI feedback follow-up | added | live observation for `ebb8b3276603a9920e6c28136677fe8536046829` found `provider-tests` failed in run `27110207713`; Codex review comments `3370406683`, `3370406685`, `3370406687` addressed. dev-coder fixed the order-independent stale assertion CI failure, added slack to the semantic-change wait test, added REST `thread_id` fallback for GraphQL `comments(last: 100)` omission, and excluded same-second review / inline review comment signals from current trigger-window feedback. dev-coder focused `uv run pytest tests/unit/infra/test_init_update.py -k pr_observation -q` = `57 passed, 226 deselected`; targeted failed test = `1 passed`; provider/mirror `diff -ru` pass; `git diff --check` pass. Final fresh code-reviewer found no code/test issue and only stale gate wording; initial final spec-reviewer found pending final gate wording and duplicate D-011 ledger id; final spec-reviewer re-run confirmed EC-010/EC-011 traceability and found only this pending gate wording. Report was corrected, and the seventh follow-up records the next live observation feedback cycle. | pass |
| main orchestrator / reviewers | PR #173 seventh live observation feedback follow-up | added | GitHub web / `gh pr checks` showed PR #173 head `2037847d4149d0b653389a4499e449cdc702ab8a` checks all pass and `mergeStateStatus=CLEAN`, but live `fetch_pr_checks_snapshot.sh` returned `ci=pending` because commit statuses API returned `aggregate_state=pending` with empty `statuses[]`. Fixed empty aggregate `pending` / `failure` / `error` as false wait / false failed backstop while preserving aggregate failure/error backstop when actual `statuses[]` entries exist. Same batch also includes same-second review collapse numeric/API-order tie-break and bootstrap-only config symlink migration guard. Main-session `pr_observation` = `59 passed`; syntax / provider-mirror parity / `git diff --check` pass; fresh code-reviewer first found empty aggregate failure and API-order fallback gaps, both fixed; fresh code-reviewer re-run pass; fresh spec-reviewer first found stale AC-004 / final gate wording, both fixed; fresh spec-reviewer re-run pass. | pass |
...
| code-reviewer | PR #173 fourth review / CI feedback follow-up diff | Codex review found P2 stale bootstrap routing in bootstrap-only `.codex/config.toml`, draft/closed snapshot gate loss in wait classification, and trigger text mention false exclusion. CI also exposed slow snapshot stability flake. Initial fresh code-reviewer found that wait semantic fingerprint could miss review body/thread changes because `review.fingerprint` was not surfaced. Fixed with exact known stale config migration, user customization preservation regression, top-level PR lifecycle `human_gate` preservation, first-nonblank-line trigger command detection, semantic wait fingerprint stability, exposed `review.fingerprint`, sanitized review signal projection that excludes raw `body`, review semantic-change regression, and raw-body-only stability regression. Final fresh code-reviewer found no issues; focused 5 tests and `pr_observation` 51 tests passed. | 2 | pass |
| code-reviewer | PR #173 fifth review feedback follow-up diff | Codex review found P2 issue comment `updated_at`, `mergeStateStatus=UNKNOWN`, commit status aggregate state, post-collection head revalidation, and `reviewDecision=CHANGES_REQUESTED`. Fresh code-reviewer found implicit no-head snapshot head drift, an order-dependent stale assertion, and edited trigger command issue comment body/artifact leakage. Fixed with collection-head revalidation for explicit/implicit head expectations, aggregate/UNKNOWN/updated_at/CHANGES_REQUESTED regressions, stable pull-review assertion selection, trigger-comment-id exclusion from current window/body artifact, and provider/mirror parity. Final fresh code-reviewer found no remaining code/test issue and reported only stale final-gate wording in this report; ledger wording was corrected. Latest focused `pr_review or pr_observation` = `71 passed`. | 4 | pass |
| code-reviewer | PR #173 sixth review / CI feedback follow-up diff | Codex review found P2 semantic-change wait test slack, P2 resolved/outdated thread state loss when GraphQL `comments(last: 100)` omits old REST comment id, and P2 same-second review/comment trigger-window false inclusion. CI also failed on order-dependent stale assertion. Fixed with longer wait-test deadline, REST `thread_id` to GraphQL thread node state fallback, same-second review/comment exclusion, and stable assertion target selection. Focused `pr_observation` = `57 passed`; targeted failed test = `1 passed`; provider/mirror parity and `git diff --check` pass. Final fresh code-reviewer found no remaining code/test issue and reported only stale final-gate wording in this report; ledger wording was corrected. | 2 | pass |
| code-reviewer | PR #173 seventh live observation feedback follow-up diff | Initial fresh code-reviewer found that empty `statuses[]` still allowed aggregate failure/error to force `ci=failed`, and that `_api_index` was removed before review collapse so API-order fallback did not actually participate. dev-coder gated aggregate failure/error with `bool(statuses)`, moved `_api_index` removal until after collapse, and added empty aggregate failure plus missing-id API-order regressions. Fresh code-reviewer re-run found no findings; reviewer spot checks `10 passed`, provider/mirror parity / `bash -n` / `git diff --check` pass. | 2 | pass |
...
| spec-reviewer | PR #173 fourth review / CI feedback spec alignment | Initial fresh spec-reviewer found P2 broad bootstrap config migration, because implementation replaced unrelated `pr-monitor` user edits despite design/plan requiring targeted migration. A later fresh spec-reviewer found P2 raw body inclusion in wait semantic fingerprint, because `review.signals` was projected wholesale despite requirement/design requiring body hashes/metadata rather than raw body text. dev-coder narrowed migration to exact known stale guidance, added preservation regression, replaced wait fingerprint `review.signals` with a sanitized projection that excludes raw `body`, and added a raw-body-only stability regression. Final fresh spec-reviewer found no issues and confirmed requirement/design/plan/report alignment. | 3 | pass |
| spec-reviewer | PR #173 fifth review feedback follow-up spec alignment | Initial fresh spec-reviewer found P1 `plan.md` EC-010 closure omission. Updated plan EC list, requirement-step traceability, closure index, and S03/S04/S05 closure contracts. Fresh spec-reviewer re-run found no blocking findings and confirmed requirement/design/plan/report EC-010 alignment. | 2 | pass |
| spec-reviewer | PR #173 sixth review / CI feedback spec alignment | EC-011 added for REST `thread_id` fallback and same-second review/comment trigger-window exclusion. Initial final spec-reviewer confirmed EC-010 / EC-011 requirement/design/plan traceability but found pending final-gate wording and duplicate D-011 ledger id in report. After correction, final fresh spec-reviewer re-run found no remaining spec consistency issue and only stale pending wording in this row / final commit row; report was corrected. | 3 | pass |
| spec-reviewer | PR #173 seventh live observation feedback spec alignment | EC-012 records empty commit-status aggregate false wait / false failed backstop, same-second review collapse, and bootstrap config symlink migration guard. Initial fresh spec-reviewer found stale AC-004 aggregate wording and pending final-gate wording; first re-run found stale Final QA Gate pending wording in earlier follow-up rows. Requirement/design/plan/report were updated to state that aggregate `pending` / `failure` / `error` is a backstop only when `statuses[]` has at least one entry, and final gate wording was closed. Fresh spec-reviewer re-run found no findings and returned `review_status: pass`. | 2 | pass |
...
| reviewer gates pass; PR #173 fourth feedback follow-up verification pass | stale bootstrap config migration, trigger command boundary, PR lifecycle human_gate preservation, semantic wait fingerprint, review semantic stability reset, raw body exclusion from wait fingerprint, focused regressions plus requirement/design/plan/report updates | final response / PR | ready_to_commit |
| reviewer gates pass; PR #173 fifth feedback follow-up verification pass | issue comment `updated_at`, UNKNOWN merge-state polling, combined status aggregate backstop, explicit/implicit post-collection head revalidation, CHANGES_REQUESTED reviewDecision, edited trigger command exclusion, stable assertion regression, and requirement/design/plan/report updates | final response / PR | ready_to_commit |
| reviewer gates pass; PR #173 sixth feedback follow-up verification pass | REST thread_id fallback, same-second review/comment trigger-window exclusion, semantic wait test slack, CI failed assertion hardening plus requirement/design/plan/report updates | final response / PR | ready_to_commit |
| reviewer gates pass; PR #173 seventh live observation feedback verification pass | empty commit-status aggregate pending/failed false-backstop fix, same-second review collapse, bootstrap config symlink migration guard, live PR #173 collector verification, and requirement/design/plan/report updates | final response / PR | ready_to_commit |
...
- 問題: PR #173 の `906f30b0d94305dfa38bdd58186364ee09753949` 再 observation 後、CI `provider-tests` が `tests/unit/infra/test_init_update.py::TestInitUpdate::test_issue_75_pr_observation_wait_counts_quiet_after_slow_snapshot_observation` で failed となり、Codex review が stale bootstrap config migration、draft/closed human gate preservation、trigger mention misclassification に関する3件の unresolved thread を返した。
  - 解決: wait same-fingerprint 判定を raw snapshot fingerprint ではなく wait decision inputs の semantic fingerprint に変更し、draft / closed PR の top-level human gate / recommended action を wait result でも preserve した。bootstrap-only `.codex/config.toml` は既知 stale guidance だけ targeted migration する。`@codex review` は explicit trigger id または first nonblank line command だけを trigger とし、本文途中の単なる言及は通常 feedback signal として残す。dev-coder focused `pr_observation` = `49 passed`; focused new regressions = `3 passed`; slow timeout regression = `1 passed`; main-session focused `pr_observation` = `49 passed`; full `test_init_update.py` = `271 passed`; syntax / provider-mirror parity / `git diff --check` は pass。その後 fresh spec-reviewer が bootstrap config migration の広すぎる置換と raw body の wait semantic fingerprint 混入を P2 として検出し、fresh code-reviewer が review semantic change を stability reset できない gap を検出した。migration を exact known stale guidance だけに絞り、`review.fingerprint` と sanitized review semantic fields を wait stability に含め、unrelated user customization 保持 / review semantic change reset / raw body-only stability の regressions を追加した。latest main-session focused `pr_observation` = `51 passed`; full `test_init_update.py` = `273 passed`; `bash -n` provider/mirror wait scripts pass; provider/mirror wait script `diff -u` pass; `git diff --check` pass。fresh spec-reviewer / code-reviewer final re-run は pass。
- 問題: PR #173 の `c17016d7c4bc37690c9f5b90e46625fc52b76e0d` 再 observation 後、CI は passed したが Codex review が issue comment `updated_at`、`mergeStateStatus=UNKNOWN`、combined status aggregate、snapshot 後 head revalidation、`CHANGES_REQUESTED` reviewDecision に関する5件の unresolved thread を返した。
  - 解決: issue comment `updated_at` を current-window/body/fingerprint に含め、`mergeStateStatus=UNKNOWN` は blocking ではなく `pending` とした。combined commit status aggregate `state` は individual statuses で失敗が見えない場合の backstop とし、snapshot collection 後の head 再検証で head drift を `stale_head` にする。GraphQL `reviewDecision=CHANGES_REQUESTED` は `changes_requested` として blocking status にする。初回 fresh spec-reviewer が見つけた `plan.md` EC-010 closure 漏れ、初回 fresh code-reviewer が見つけた `--head-sha` 未指定 snapshot の collection-head drift、後続 fresh code-reviewer が見つけた order-dependent stale assertion と edited trigger command body/artifact leakage を追加修正した。latest focused `pr_review or pr_observation` = `71 passed`; review script `bash -n` provider/mirror pass; provider/mirror parity / `git diff --check HEAD~1..HEAD` は pass。final fresh code-reviewer は stale final-gate wording のみを指摘し、本 report を修正した。final fresh spec-reviewer re-run は pass。
- 問題: PR #173 の `ebb8b3276603a9920e6c28136677fe8536046829` 再 observation 後、CI `provider-tests` が `test_issue_170_pr_review_collector_changes_requested_decision_is_blocking` で failed となり、Codex review が semantic-change wait test slack、GraphQL `comments(last: 100)` thread-state fallback、same-second review/comment trigger-window tie-break に関する3件の unresolved thread を返した。
  - 解決: CI failure は signal 順序依存 assertion を `kind` / `id` に基づく対象 selection へ変更した。semantic-change wait test は deadline slack を増やした。REST review comment の `thread_id` を GraphQL thread node id と突合し、GraphQL comments window に古い comment id がない場合でも thread state を反映する。review / inline review comment の `created_at` / `updated_at` が trigger と同秒の場合は current trigger-window signal から除外する。dev-coder focused `pr_observation` = `57 passed`; targeted failed test = `1 passed`; provider/mirror parity / `git diff --check` は pass。final fresh code-reviewer は pass。initial final spec-reviewer が見つけた pending gate wording と duplicate D-011、final fresh spec-reviewer re-run が見つけた stale pending wording は本 report で修正した。
- 問題: PR #173 の `2037847d4149d0b653389a4499e449cdc702ab8a` live observation 中、GitHub web / `gh pr checks` は checks all pass / `mergeStateStatus=CLEAN` を示していたが、CI collector は commit statuses API の empty `statuses[]` + `aggregate_state=pending` を pending backstop として扱い続け、`ci=pending` のままになった。
  - 解決: aggregate `pending` / `failure` / `error` は実 commit status context がある場合だけ backstop とし、empty `statuses[]` + green check runs / required rollup では `ci=passed` にできるようにした。併せて同秒 review collapse の API-order fallback を実効化し、bootstrap-only config symlink migration skip を regression で固定した。main-session `pr_observation` = `59 passed`; syntax / provider-mirror parity / `git diff --check` pass; fresh code-reviewer first fail findings were fixed; fresh code-reviewer re-run pass; initial fresh spec-reviewer findings were fixed in AC-004 / final gate wording; fresh spec-reviewer re-run pass.
- 問題: PR #173 の latest head `613bb0b10ad01b25f5866f0d4ed637de3536d330` push 後、GitHub checks / merge state は clean だが、過去 Codex review thread が GitHub 上で unresolved のまま残っていた。
  - 解決: `wait_pr_observation.sh --repo chemitaro/spec-dock --pr 173 --head-sha 613bb0b10ad01b25f5866f0d4ed637de3536d330 --timeout-seconds 1800 --poll-interval-seconds 30 --quiet-seconds 60 --same-fingerprint-count 2` を実行し、`ci=passed`、`mergeStateStatus=CLEAN`、`head_matches_expected=true`、`review=unresolved`、`normalized_status=human_gate`、`recommended_next_action=address_review_feedback` を確認した。`gh pr checks 173` は provider-tests / validate とも pass。thread-aware read では unresolved 27 件、うち non-outdated 15 件 / outdated 12 件を確認した。内容は過去 follow-up で実装・テスト・fresh reviewer pass 済みだが、GitHub thread state の解決は未実施。
...
## 省略/例外メモ (必須)
- PR #173 で live `wait_pr_observation.sh` / collector を実行し、`ebb8b3276603a9920e6c28136677fe8536046829` では CI failed / review unresolved、`2037847d4149d0b653389a4499e449cdc702ab8a` では GitHub checks passed だが旧 collector が empty aggregate pending で false wait することを確認した。修正後は empty aggregate `pending` / `failure` / `error` の false backstop を防ぎ、main-session `pr_observation` = `59 passed`、fresh code-reviewer re-run pass、fresh spec-reviewer re-run pass まで確認済み。latest head `613bb0b10ad01b25f5866f0d4ed637de3536d330` の外部 PR observation は CI passed / mergeStateStatus CLEAN / review unresolved / human_gate で完了した。
