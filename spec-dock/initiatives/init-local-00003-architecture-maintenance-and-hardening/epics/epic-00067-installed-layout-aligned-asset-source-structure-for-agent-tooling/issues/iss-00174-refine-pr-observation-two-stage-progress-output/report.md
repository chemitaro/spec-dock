...
| D-005 | resolved | implementation | Codex review / orchestrator | ユーザーが `@codex review` を投稿した後、PR #175 に inline review 3件が追加された。指摘は raw `review.fingerprint` による quiet reset、body-mode omission で `comments=N` が 0 になる問題、stale signal counting の3点。 | A: advisory として修正しない; B: 3件とも current review progress / quiet semantics の実害として修正する | B を採用する。body 出力 mode / cap による omission は count 対象に残し、trigger/window/timestamp 不明、stale、non-Codex、trigger command は除外する。wait fingerprint から raw collector `review.fingerprint` を外す。 | 3件とも「progress 表示と quiet reset は同じ current projection を共有する」という issue の中核意図に関わる。特に PR #175 の実観測で `--body-mode none` では Codex comment があっても `comments=0` になっていた。 | applied | `discussions/20260608t073119z-research-codex-review-feedback-analysis.md`; `src/spec_dock/assets/install_root/.agents/skills/github-pr-observation/scripts/wait_pr_observation.sh`; `.agents/skills/github-pr-observation/scripts/wait_pr_observation.sh`; `tests/unit/infra/test_init_update.py`; focused pytest -> `82 passed, 213 deselected` | push後に `@codex review` を再投稿し、PR checks / review を再確認する |
| D-006 | resolved | implementation | Codex review / orchestrator | 再 push 後の `@codex review` で、`body_mode_none` / `body_mode_out_only` allowlist が trigger-window 判定前の omission reason を current と誤認し、古い Codex signal を `comments=N` に混入させ得ると指摘された。 | A: allowlist のまま advisory 扱い; B: collector の current status 判定を signal metadata として wait 側へ渡し、progress count / quiet reset の current 判定へ使う | B を採用する。collector で `current_status_signal` を付与し、wait 側はこの明示フラグを優先する。body-mode omission は current signal に限って count し、フラグ未存在時のみ旧形式 snapshot 互換 fallback を使う。 | D-002 の `comments=N` 定義は current trigger-window Codex review signal 件数であり、`omitted_reason` は body 出力境界であって current 性の根拠ではない。 | applied | `discussions/20260608t073119z-research-codex-review-feedback-analysis.md`; `src/spec_dock/assets/install_root/.agents/skills/github-pr-observation/scripts/lib/fetch_pr_review_snapshot.sh`; `src/spec_dock/assets/install_root/.agents/skills/github-pr-observation/scripts/wait_pr_observation.sh`; `tests/unit/infra/test_init_update.py`; focused pytest -> `83 passed, 213 deselected` | push / rerun `@codex review` |
| D-007 | resolved | implementation | Codex review / orchestrator | 再レビューで、`semantic_fingerprint()` が progress 用の Codex-authored signal projection だけを使うため、人間 reviewer など non-Codex current review change で quiet が reset されないと指摘された。 | A: `comments=N` と fingerprint を完全に同じ Codex-only projection に保つ; B: progress count と semantic fingerprint の projection を分離し、fingerprint は current review semantic signals 全体を見る | B を採用する。`comments=N` は Codex-authored current signal のまま維持し、semantic fingerprint には `current_status_signal` で絞った review semantic signals を Codex authored 여부に関係なく含める。 | R-001 の目的は raw collector fingerprint の noise を外すことであり、current な人間レビュー変更まで無視することではない。PR observation の wait stability は current semantic state の変化に反応すべきである。 | applied | `discussions/20260608t073119z-research-codex-review-feedback-analysis.md`; GitHub discussion `3371654171`; `src/spec_dock/assets/install_root/.agents/skills/github-pr-observation/scripts/wait_pr_observation.sh`; `tests/unit/infra/test_init_update.py`; focused pytest -> `84 passed, 213 deselected` | push / rerun `@codex review` |
...
| EAL-007 | adopted | Codex review analysis + dev-coder fix | source / tests / `report.md` | `@codex review` の3件は設計意図と実観測に照らして妥当であり、current review signal projection の補正として採用した。 | `discussions/20260608t073119z-research-codex-review-feedback-analysis.md`; `/private/tmp/iss-00174-dev-coder-codex-review-fix.json`; focused pytest -> `82 passed, 213 deselected` | push and rerun `@codex review` |
| EAL-008 | adopted | follow-up Codex review analysis | source / tests / `report.md` | 再レビューの `current_status_signal` 指摘は、body omission と trigger-window current 判定を混同しないために必要であり、D-002 の comments 定義を守る evidence として採用した。 | `discussions/20260608t073119z-research-codex-review-feedback-analysis.md`; GitHub discussion `3371528181`; focused pytest -> `83 passed, 213 deselected` | push and rerun `@codex review` |
| EAL-009 | adopted | follow-up Codex review analysis | source / tests / `report.md` | 再レビューの non-Codex semantic change 指摘は、progress 表示の Codex-only 集計と wait stability の current semantic state tracking を分離する必要を示す evidence として採用した。 | `discussions/20260608t073119z-research-codex-review-feedback-analysis.md`; GitHub discussion `3371654171`; focused pytest -> `84 passed, 213 deselected` | push and rerun `@codex review` |
...
| S99 | dev-coder / orchestrator | Addressed Codex review comments by aligning review progress counts with current Codex signals, adding explicit `current_status_signal` metadata, and separating review semantic fingerprint projection from progress count projection. | `src/spec_dock/assets/install_root/.agents/skills/github-pr-observation/scripts/lib/fetch_pr_review_snapshot.sh`; `.agents/skills/github-pr-observation/scripts/lib/fetch_pr_review_snapshot.sh`; `src/spec_dock/assets/install_root/.agents/skills/github-pr-observation/scripts/wait_pr_observation.sh`; `.agents/skills/github-pr-observation/scripts/wait_pr_observation.sh`; `tests/unit/infra/test_init_update.py` | `bash -n` scripts -> pass; `diff -u` -> empty; `git diff --check` -> pass; focused pytest -> `84 passed, 213 deselected`; `spec-dock validate` -> pass | post-fix PR checks/review pending after push | none known | accepted for push and review rerun |
...
- 問題: `@codex review` 後、body-mode omission が `comments=N` を 0 にする、stale signal が count される、raw collector `review.fingerprint` が filtered noise で quiet reset を起こす、という3件の指摘があった。
  - 解決: review progress signal predicate を current signal semantics に補正し、wait fingerprint から raw `review.fingerprint` を外し、body-mode omission / stale / raw fingerprint drift の regression tests を追加した。
