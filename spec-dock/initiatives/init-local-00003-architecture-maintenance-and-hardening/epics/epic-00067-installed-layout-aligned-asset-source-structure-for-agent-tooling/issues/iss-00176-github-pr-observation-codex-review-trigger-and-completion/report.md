...
- 問題: PR #177 再 push 後の Codex review で、snapshot fallback issue comment の扱い、final wait JSON の trigger metadata 欠落、pre-trigger unresolved thread id の混入が指摘された。また GitHub Actions provider-tests では timeout limitation source の環境差で既存テストが失敗した。
  - 解決: snapshot direct entrypoint でも `fallback_issue_comment` を human gate にし、wait final JSON に post-once trigger helper metadata を保持した。review thread summary / status は selected trigger boundary thread に限定し、timeout limitation の source 期待を環境差に耐える形へ更新した。
  - 証跡: PR #177 observation r2 -> CI failed 1 / review unresolved 2 selected comments / `recommended_next_action=fix_ci`。`uv run pytest tests/unit/infra/test_init_update.py -k "issue_176 or test_issue_75_pr_observation_wait_requires_quiet_and_same_fingerprint or test_issue_170_pr_observation_snapshot_derives_human_gate_from_review_feedback or test_issue_75_pr_observation_review_collector_uses_thread_comment_activity_for_current_window or test_issue_170_pr_review_collector_excludes_pre_trigger_unresolved_threads_from_status"` -> 29 passed。`uv run pytest tests/unit/infra/test_init_update.py` -> 322 passed。`./spec-dock/scripts/spec-dock validate` -> ok nodes=90。`git diff --check` -> pass。
- 問題: PR #177 r3 の Codex review で、draft / non-open PR でも trigger helper が POST まで進むこと、trigger timeout が helper の子プロセスを残し得ること、trigger window 内の actionable feedback が Codex submitted review completion 前に timeout / resume 待機へ隠れることが指摘された。
  - 解決: `trigger_codex_review.sh` は head 一致後かつ POST 前に draft / non-open PR を `skipped` として返すようにした。`wait_pr_observation.sh` の trigger 実行は snapshot と同じ process group timeout 終了経路へ寄せ、descendant process の遅延副作用を止める regression を追加した。`commented` / `changes_requested` / `unresolved` / `requested` は fallback issue comment を除き、Codex submitted review completion 前でも `human_gate` / `address_review_feedback` に返すよう wait / snapshot の分類順を揃えた。
  - 証跡: PR #177 observation r3 -> CI passed / review unresolved 3 selected comments / `recommended_next_action=address_review_feedback`。`uv run pytest tests/unit/infra/test_init_update.py -k "issue_176 or test_issue_75_pr_observation_wait_requires_quiet_and_same_fingerprint or test_issue_170_pr_observation_snapshot_derives_human_gate_from_review_feedback or test_issue_75_pr_observation_review_collector_uses_thread_comment_activity_for_current_window or test_issue_170_pr_review_collector_excludes_pre_trigger_unresolved_threads_from_status or test_issue_170_pr_observation_wait_preserves_snapshot_human_gate_for_draft_and_closed_pr"` -> 32 passed。`uv run pytest tests/unit/infra/test_init_update.py` -> 324 passed。`./spec-dock/scripts/spec-dock validate` -> ok nodes=90。`git diff --check` -> pass。provider/mirror `fetch_pr_observation_snapshot.sh` / `trigger_codex_review.sh` / `wait_pr_observation.sh` parity -> match。
- 問題: PR #177 r4 の Codex review で、trigger POST / recovery 成功後の最終 `gh pr view` が一時失敗すると有効な trigger metadata を失い duplicate trigger の原因になること、`fallback_issue_comment` terminal で `wait_or_resume` を返しても `resume.command_hint` が final JSON に含まれないことが指摘された。
  - 解決: POST / recovery 成功済みの場合、post-trigger metadata revalidation failure は warning limitation として記録し、`comment_id` / `created_at` を保持して観測へ進めるようにした。wait loop は `next_action=wait_or_resume` の terminal / timeout 経路で trigger metadata merge 後に `resume` metadata を付与するようにした。
  - 証跡: PR #177 observation r4 -> CI passed / review unresolved 2 selected comments / `recommended_next_action=address_review_feedback`。`uv run pytest tests/unit/infra/test_init_update.py -k issue_176` -> 28 passed。`uv run pytest tests/unit/infra/test_init_update.py -k "issue_176 or test_issue_75_pr_observation_wait_requires_quiet_and_same_fingerprint or test_issue_170_pr_observation_snapshot_derives_human_gate_from_review_feedback or test_issue_75_pr_observation_review_collector_uses_thread_comment_activity_for_current_window or test_issue_170_pr_review_collector_excludes_pre_trigger_unresolved_threads_from_status or test_issue_170_pr_observation_wait_preserves_snapshot_human_gate_for_draft_and_closed_pr"` -> 33 passed。`uv run pytest tests/unit/infra/test_init_update.py` -> 325 passed。`./spec-dock/scripts/spec-dock validate` -> ok nodes=90。`git diff --check` -> pass。provider/mirror `trigger_codex_review.sh` / `wait_pr_observation.sh` parity -> match。
