...
| `wait_pr_observation.sh` current boundary re-review | passed snapshot の quiet/stability 待ち中に wait-local completion を decision fingerprint へ混ぜる | passed decision では nested decision completion/fingerprint を書き換えず、feedback gate など non-pass decision だけ top-level wait completion と同期する | `uv run pytest tests/unit/infra/test_init_update.py -k "issue_75_pr_observation_wait_completes_after_stable_fingerprint_and_quiet or issue_182"` -> 17 passed | remediated |
| `fetch_pr_review_snapshot.sh` current boundary re-review | global `reviewDecision=CHANGES_REQUESTED` を current evidence に昇格していた | global reviewDecision fallback を削除し、current-boundary selected review/comment のみ `selected_changes_requested_evidence` にする | `uv run pytest tests/unit/infra/test_init_update.py -k "issue_75_pr_observation_wait_completes_after_stable_fingerprint_and_quiet or issue_182"` -> 18 passed | remediated |
| `wait_pr_observation.sh` current boundary re-review | timeout 時に passed nested decision が残り得る | timeout 補正を passed decision に限定して `timeout` / `wait_or_resume` に落とし、pending decision の fingerprint stability は維持 | `uv run pytest tests/unit/infra/test_init_update.py -k "issue_75_pr_observation_wait_completes_after_stable_fingerprint_and_quiet or issue_182"` -> 18 passed | remediated |
| `wait_pr_observation.sh` current boundary re-review | timeout 時の nested decision 補正が prior status に依存していた | timeout 結果では prior status に関係なく nested decision を `timeout` / `wait_or_resume` に更新し、poll 中の decision fingerprint stability は events で確認する | `uv run pytest tests/unit/infra/test_init_update.py -k "issue_182_s03_wait_uses_decision_fingerprint_for_audit_only_changes or issue_182_s03_wait_timeout_clears_nested_passed_decision or issue_75_pr_observation_wait_completes_after_stable_fingerprint_and_quiet or issue_182"` -> 18 passed | remediated |
