...
- 具体テストケース:
  ...
  | tc-s01-005 | cl-001 | unit / negative | fake sync result の warning code が `gh_index_incomplete` | post-mutation sync helper を実行 | `failed == true`、incomplete GitHub index と recovery guidance を持つ | warning のまま success 扱いなら fail |
