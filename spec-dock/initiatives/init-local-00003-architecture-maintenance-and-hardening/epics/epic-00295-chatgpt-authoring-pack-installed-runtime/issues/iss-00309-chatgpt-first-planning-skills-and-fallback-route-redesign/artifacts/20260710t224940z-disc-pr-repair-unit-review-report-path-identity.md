---
種別: disc
ID: "20260710t224940z-disc"
タイトル: "PR Repair Unit U015 Review Report Path Identity"
...
# 20260710t224940z-disc PR Repair Unit U015 Review Report Path Identity

## PR Repair Unit

- source_batch: `20260710t122133z-pr-repair-batch-pr-311-repair-batch.md`
- unit_id: `U015`
- root_cause_family: `authoring-pack.effective-input-boundary`
- covered_ids: `R024`
- decided_priority: `P1`
- merge_blocking: `yes`
- disposition: `fix-now`
...
- 根本原因: review reportの字句参照経路と後段が読み取る証跡identityを一致させていない。
- 設計: 入力証跡専用helperでtrusted root containment、leaf/ancestor symlink、strict resolveを確認し、同一bytesからJSONとdigestを算出する。
- 実装手順: shared evidence reader、candidate/draft caller、CLI回帰テスト、provider/dogfood mirrorの順に変更する。
- 検証: candidate/issue draft/selected skeletonでleaf・ancestor・broken・root外を拒否し、root内通常ファイルを許可する。
