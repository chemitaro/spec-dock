---
種別: disc
ID: "20260710t235246z-disc"
タイトル: "PR Repair Unit U016 Provider Static Gate"
...
# 20260710t235246z-disc PR Repair Unit U016 Provider Static Gate

## PR Repair Unit

- source_batch: `20260710t122133z-pr-repair-batch-pr-311-repair-batch.md`
- unit_id: `U016`
- root_cause_family: `provider-ci.final-static-gate-omission`
- covered_ids: `R025`
...
- 根本原因: 最終push前にCIと同じ`make lint`を実行せず、限定ruff checkとpytestだけで完了判定した。
- 症状: 3ファイルのformat漏れと、`candidate_validation._review_gate`内の異なる意味を持つ`status`変数再利用によるmypy衝突。
- 設計: 指摘3ファイルへ機械formatを適用し、失敗結果用変数を`failure_status`へ改名する。castや追加ignoreは使わない。
- 検証: provider/dogfood一致、focused test、`make lint`、Provider CI再観測。
- result: `implemented; make lint passed, authoring focused 385 passed, validate nodes=203`
- commit: `pending`
