---
種別: disc
ID: "20260710t122142z-disc"
タイトル: "PR Repair Unit Windows Path Boundary"
...
# 20260710t122142z-disc PR Repair Unit Windows Path Boundary
...
- source_links: PR review comments 3558754353, 3558754356
- failure_class: `review_feedback:windows-path-boundary`
- risk_class: blocking
- disposition: fix-now

## Validity Analysis

Both findings are valid. Drive-qualified and backslash-based names can acquire
host-specific absolute/traversal semantics after a later platform `Path` join.

## Need-To-Fix Decision

The reviewed evidence boundary must be portable and contained, so both forms
must be rejected before staging or candidate validation can pass.

## Root Cause

Two validators used `PurePosixPath` without first applying the portable path
checks already used by candidate index paths.

## Options Considered

- Resolve after joining: platform-dependent and too late.
- Reject backslashes and drive prefixes lexically: deterministic and portable.

## Recommended Design

Apply the same lexical backslash and drive-prefix checks before
`PurePosixPath` parsing in ZIP entry and draft-file validation.

## Implementation Plan

1. Extend both validators.
2. Add ZIP and candidate draft regression cases.
3. Run focused and full provider tests.

## Validation Plan

- Unsafe Windows drive/backslash forms return rejected findings.
- Existing safe POSIX-relative inputs continue to pass.
...
Low; the repair is lexical and does not change valid portable inputs.
