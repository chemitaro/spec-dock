---
種別: disc
ID: "20260710t224939z-02-disc"
タイトル: "PR Repair Unit U014 Backend Stream Evidence Minimization"
...
# 20260710t224939z-02-disc PR Repair Unit U014 Backend Stream Evidence Minimization

## PR Repair Unit

- source_batch: `20260710t122133z-pr-repair-batch-pr-311-repair-batch.md`
- unit_id: `U014`
- root_cause_family: `authoring-pack.sensitive-output-retention`
- covered_ids: `R023`
- decided_priority: `P1`
- merge_blocking: `yes`
- disposition: `fix-now`
...
- 根本原因: 任意backend streamを安全にredactできるという前提でdurable summaryへ保存している。
- 設計: stdout/stderr本文は永続化しない。互換キーは空文字で維持し、byte countと`not_persisted` dispositionだけを保存する。
- 実装手順: result contract、application caller、renderer/summary回帰テスト、provider/dogfood mirrorの順に変更する。
- 検証: success/nonzero/timeoutでsecret・会話本文がCLIとsummaryに残らず、件数・exit code・statusが残ることを検証する。
- result: `implemented; authoring focused 385 passed, unit 968 passed, full 2301 passed`
- commit: `pending`
