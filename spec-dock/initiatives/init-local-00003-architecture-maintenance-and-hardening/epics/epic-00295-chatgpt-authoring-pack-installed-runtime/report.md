...
- 現在地（何が完了し、何が未完か）:
  ...
  - 現在ブランチを commit / push した上で、ChatGPT-Use / GPT-5.5 Pro Extended に ZIP 形式の final authoring pack を生成させ、Epic 正本候補 3 本と 12 Issue draft pack を取得した。
  - ZIP 由来の Epic 正本候補本文を `requirement.md` / `design.md` / `plan.md` へ採用し、Issue draft pack は Issue node 作成前の evidence-only pack として `artifacts/20260707t164532z-chatgpt-final-authoring-pack/` に保存した。
  - ChatGPT final authoring pack の 12 Issue 候補を実際の SpecDock Issue node `iss-00296` 〜 `iss-00307` として作成した。
  - 各 Issue に `draft-requirement` / `draft-design` / `draft-plan` artifact を作成し、ChatGPT ZIP 由来 draft 本文を Issue-local artifact へ配置した。
  - `iss-00297 -> iss-00296` から `iss-00307 -> iss-00306` まで、リレー実装順の Issue dependency edge を追加した。
- 次のマイルストーン:
...
| EAL-010 | 部分採用（`partially_adopted`） | ChatGPT-Use / GPT-5.5 Pro Extended ZIP authoring pack（`delegated_draft`） | Epic requirement / design / plan finalization candidate / Issue draft handoff evidence | commit / push 済みの branch を ChatGPT に参照させ、ZIP 形式で Epic 正本候補 3 本と 12 Issue draft pack を取得した。Epic 正本候補の本文は main orchestrator が front matter を維持して `requirement.md` / `design.md` / `plan.md` へ採用した。一方で、Issue draft pack は Issue Decomposition Approval Gate 前であり、canonical Issue docs ではなく evidence-only pack として保存した。 | `artifacts/20260707t164856z-research-chatgpt-final-zip-authoring-pack-adoption.md`, `artifacts/20260707t164532z-chatgpt-final-authoring-pack/` | 12 Issue 候補の人間承認後、Issue node を作成し、各 Issue の `spec-dock-issue-planning draft-adoption` 入力として個別採用する |
| EAL-011 | 採用（`adopted`） | ChatGPT ZIP issue draft pack（`delegated_draft`） | Issue node creation / Issue-local draft artifact handoff | ChatGPT final authoring pack の 12 Issue 候補を SpecDock Issue node `iss-00296` 〜 `iss-00307` として作成し、各 Issue の `artifacts/` に `draft-requirement` / `draft-design` / `draft-plan` を配置した。`draft-design` / `draft-plan` は各 Issue の assurance contract を `standard` profile で classify / verify した後に作成した。Issue dependency edge はリレー実装順に追加済み。 | `artifacts/20260707t164532z-chatgpt-final-authoring-pack/adopted-issue-index.json`, `issues/iss-00296-promote-authoring-pack-assets/` ... `issues/iss-00307-final-quality-gate-and-mergeable-pr-delivery/` | 各 Issue start 時に `spec-dock-issue-planning draft-adoption` で Issue-local draft artifact を採否判断し、canonical Issue docs へ正本化する |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ChatGPT-Use / GPT-5.5 Pro Extended | epic-00295 | `artifacts/20260707t164532z-chatgpt-final-authoring-pack/` | commit `4cc193a3c0fb69a860108c9c3f61f0a4a24d1757`, `/private/tmp/codex-agent-work/501/session-20260707t162117z-epic00295-chatgpt-zip-pack-382f9dea/prompt.md`, `/Users/iwasawayuuta/.oracle/sessions/epic00295-final-pack-zip/artifacts/epic-00295-chatgpt-authoring-final-pack.zip` | `requirement.md`, `design.md`, `plan.md`, future Issue docs after approval | 未レビュー（`unreviewed`） | [`requirement.md`, `design.md`, `plan.md`] | ZIP file / SHA-256 / extracted tree checked; SpecDock validation passed | Epic 正本候補本文は採用、Issue drafts は evidence-only handoff | ChatGPT front matter、canonical authority claim として見える表現、Issue node 作成前の draft 正本化 | なし（none） | 未実施 | Epic docs は正本反映済み。Issue draft pack は human approval gate 後に個別採用 |
