...
| D-005 | resolved | scope / operation | S90 ChatGPT Pro review / orchestrator | S04でaccepted provider runtime sourceがcandidate-wheel disposable consumerへ投影済みだが、チェックイン済みdogfood mirrorが遅れており、S90で同期した4 pathsが「runtime repair」と誤認され得た | revert mirror and accept stale checked-in dogfood; plan amendmentで既存provider差分のmanaged projection correctionを明示; new runtime repair | candidate-wheel updateが実測で出力する既存managed mirrorだけをprovider-firstに同期し、production repair=false・originating step=S04・status manifestを保持する。S90はruntime挙動を変更せず、projection parityとreport traceだけを完了する。Issue 345のprovider source provenanceは`1e276af98cfad7a1b3bcf82d569c44dab4dad2a7`、`f9b94d6adb1f99def916c40166e45f8c90103bc4`、`e001c404ca33bf337255d46b3c16d64fb0df7fdf`で固定する | S04 `I346-AC-006` / `Provider-to-Dogfood Projection Manifest`、`870846fd`、`6b815c9c`、Issue 345 provider commits、ChatGPT S90 finding P1-1 | promoted_to_plan | `plan.md` §12.3 clarification、Issue EAL-013/EAL-014、S04 report projection receipt | fresh S90 combined code/spec reviewでexact-head scopeを再確認する |
| D-006 | resolved | scope / test-strategy | S90 ChatGPT Pro re-review / orchestrator | S04 projectionで生成されたdocs pathを期待status manifestへ反映したテスト差分が、S90 docs-only allowlist外に見える状態だった | revert status-manifest edit; S90で新規testを追加; originating S04 status-manifest correctionを明示委任 | `tests/integration/test_epic_00343_distribution.py`の`_S04_UPDATE_EXPECTED_STATUS`追加だけをS04 originating-stepのprojection receiptとして認可する。新しい挙動・assertionは含めず、S04 status evidenceとして扱い、dev-coder委任とcurrent-head combined code/spec reviewを必須にする | `6b815c9c`、S04 status manifest、ChatGPT S90 re-review finding、plan §12.3/§12.4 amendment | promoted_to_plan | `plan.md` §12.3/§12.4、Issue EAL-014 | S90 combined reviewでexact-head test scopeとreport bindingを確認する |
...
| EAL-014 | `adopted` | S04 status-manifest correction (`6b815c9c`) | S90 plan/report scope authorization and S04 closure trace | `tests/integration/test_epic_00343_distribution.py`の変更は、S04 candidate updateが出力するdocs pathを`_S04_UPDATE_EXPECTED_STATUS`へ追加するstatus-only補正であり、S90の新規test behaviorではない。S90でこのexact pathを明示認可し、dev-coder委任とcombined code/spec reviewを追加条件にする | `6b815c9c35a3a149450ca61847c228dadd1d9ad6`; S04 report status manifest; plan §12.3/§12.4; current test commit `d5c98f858f26288dfc322a2fd83516debe51f8a9` only formatting/type narrowing | plan §12.3/§12.4へpromoted。S04 originating scopeを保持し、S90 combined reviewでexact-head bindingを取得する | blocking: no; adopter: main orchestrator; reviewer: pending S90 ChatGPT Pro combined review; next_action: S90 combined review |
