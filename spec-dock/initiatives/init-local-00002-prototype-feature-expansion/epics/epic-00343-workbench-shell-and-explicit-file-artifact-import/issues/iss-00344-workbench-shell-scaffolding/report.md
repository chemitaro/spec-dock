...
| plan | approved requirement/design、ChatGPT planning candidate、Standard assurance obligations、current provider/build/test seams | B-001/B-002をcanonical planへ修正済み。3つのvertical micro-batch、generic exact-copy、exact distribution/static commands、Closure、TDD、evidence、stop/handoffを正規化 | partially_adopted | failed (ChatGPT advisory B-001/B-002; fixes applied) | yes | assurance再分類、commit / push 後にfresh ChatGPT plan re-review、その後fresh `spec-reviewer` plan review |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 該当なし | iss-00344 | 該当なし | 該当なし | 該当なし | not used | [] | not_run | delegated draft lane 未使用。ChatGPT preserved external evidence は Evidence Adoption Ledger の EAL-001〜EAL-006 で管理 | 該当なし | なし | fresh `spec-reviewer` PASS | delegated draft promotion なし |
