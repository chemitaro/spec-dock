...
| S90 test lane | dev-coder | provider 4文書のshared/role-specific guidanceを一つのsemantic assertionへ集約し、canonical READMEをread-only preconditionとしてvalid Redを固定 | `tests/unit/infra/test_init_update.py` | exact node `1 failed`（stale semantics 37件のみ）、canonical asset `1 passed`、Ruff/format/diff-check pass。default lane `1 skipped`はpolicyどおり | fresh ChatGPT-Use `code-reviewer`責務review pending | Issue 345/346 availability変更時はdocsとtestの同時更新が必要 | accepted as review candidate; docs laneはreview PASSまで未admitted |
...
| S90 test lane | test-only candidate review | code-reviewer | pending | pending | no | review exact pushed candidate | 正規ChatGPT-Useラッパーでblocking/major 0を確認するまでdocs laneへ進まない |
...
| S90 test lane | candidate | exact aggregate semantic test、EAL-040、delegation/Red evidence | commit/push後にexact review targetを記録 | pending | not applicable | not applicable | not applicable | not applicable |
