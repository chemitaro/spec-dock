...
| D-012 | resolved | operation | ChatGPT-Use S99 review execution / parent orchestrator | review target `401015350f4f9f82d104f4cda1064c0edfa722fb`へのfresh spec/QA reviewはPASSしたが、code reviewは`prompt-commit-timeout`となり、`promptSubmitted=true`、`conversationId=null`、assistant/user turnなし、project homeだけが回収された。送信commitmentが不確実な同一HEADへ再送できない | 同一HEADへ再送; incomplete reviewを採用; sub-agentへ代替; report-only recovery evidence commitで新review targetを作る | 同一HEADのcode reviewは採用せず、同HEADのspec/QA outputsもbranch mutationによりstaleとする。失敗事実だけをreportへ記録した新review targetをpushし、aggregate verificationと三者fresh ChatGPT-Use reviewを新HEADで直列再実行する | prompt重複を防ぎつつ、planのbranch-changing repair stale-head契約と三者独立reviewを維持できる。実装・test・approved specは変更しない | applied | Oracle sessions `iss-00344-s99-code-review-2`, `iss-00344-s99-qa-review-3`, `iss-00344-s99-spec-review`; canonical wrapper diagnostics / same-session `--harvest --no-recover` / `--live`; old review head `401015350f4f9f82d104f4cda1064c0edfa722fb` | report-only recovery commitをpushし、新HEADでS99 aggregate verificationとfresh QA/code/spec reviewsを再実行する |
