...
|---|---|---|---|---|---|---|
| requirement | 親 Epic requirement/design/plan、provider source、package config、relevant tests、ChatGPT authoring/review Artifacts | F-001〜F-004 の修正後 re-review で F-005/F-006 を検出。managed provider update 境界と premature promotion claim を canonical docs に修正済み | partially_adopted | failed (`F-005`, `F-006`; F-001/F-003 は解消確認、F-002 は F-005 として残存修正済み) | yes | commit / push 後に fresh ChatGPT re-review、その後 fresh `spec-reviewer` review |
