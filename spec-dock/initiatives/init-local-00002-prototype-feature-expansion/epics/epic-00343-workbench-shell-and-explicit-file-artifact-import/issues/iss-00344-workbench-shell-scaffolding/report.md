...
| D-013 | resolved | implementation | PR #350 Codex review / ChatGPT-Use consultation / parent orchestrator | exact head `7bac5ee606e235df52c3a23be433fba3cdbf491a`のPR observationでP1 `uninstall-managed-inventory`。fresh initが生成するroot `.workbench/README.md`が`_build_scaffold_uninstall_sources()`に未登録で、unchanged managed fileがunmanagedとして保存される。review本文はretry markerも残ると述べた | finding全却下; `.workbench`再帰削除; exact target/source mapping; marker lifecycle変更 | core P1をvalidとして採用し、既存exact-match inventoryへroot READMEのtarget/source mappingを1件追加する。modified READMEとarbitrary payloadは保存する。marker残存は既存idempotent rerun契約であり原因ではないため、marker削除/reorderは却下 | generation/uninstall ownershipを対称化しつつ、Workbench opacityとdestructive-operation safetyを維持する最小修正である | applied | PR #350 review comment `3675040563`; repair batch `artifacts/20260729t141008z-pr-repair-batch-pr-350-repair-batch-1.md`; U001 discussion; ChatGPT consultation Artifact EAL-053; Red `4 failed`; Green/aggregate EAL-054 | repair candidateをcommit/pushし、新exact headでaggregate verificationとfresh QA/code/spec reviewsを再実行する |
