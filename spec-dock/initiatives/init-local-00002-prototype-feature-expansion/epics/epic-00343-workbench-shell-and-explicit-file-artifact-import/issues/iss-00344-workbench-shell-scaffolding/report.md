...
|---|---|---|---|---|---|---|
| requirement | 親 Epic requirement/design/plan、provider source、package config、relevant tests、ChatGPT authoring/review Artifacts | ChatGPT review F-001〜F-004: runtime command 表記、no-backfill 範囲、source-wins copy 条件、report lifecycle state。canonical docs に修正済み | partially_adopted | failed (`F-001`〜`F-004`) | yes | commit / push 後に fresh ChatGPT re-review、その後 fresh `spec-reviewer` review |
