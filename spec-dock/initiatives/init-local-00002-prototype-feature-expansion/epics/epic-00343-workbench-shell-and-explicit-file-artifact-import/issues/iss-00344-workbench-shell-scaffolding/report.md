...
| S90 test lane | dev-coder | provider 4文書のshared/role-specific guidanceを一つのsemantic assertionへ集約し、canonical READMEをread-only preconditionとしてvalid Redを固定 | `tests/unit/infra/test_init_update.py` | exact node `1 failed`（stale semantics 37件のみ）、canonical asset `1 passed`、Ruff/format/diff-check pass。default lane `1 skipped`はpolicyどおり | initial fresh review `FAIL`: CR-S90-TEST-001 blockingを採用 | Issue 345/346 availability変更時はdocsとtestの同時更新が必要 | bounded test-only fix and fresh re-review; docs lane未admitted |
...
| S90 test lane | candidate | exact aggregate semantic test、EAL-040〜042、delegation/Red/review evidence | superseded targets `74e4362ac16508c3d6db21eb62d6289ece1d4379`, `c043bad10e42d9c023f84bb0fc29dacaf8614863`; second bounded fix pending | pending | not applicable | not applicable | not applicable | not applicable |
