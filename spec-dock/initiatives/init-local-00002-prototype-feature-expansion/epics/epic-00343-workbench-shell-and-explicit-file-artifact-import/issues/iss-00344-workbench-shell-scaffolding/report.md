...
| plan | approved requirement/design、ChatGPT planning candidate、Standard assurance obligations、current provider/build/test seams、Issue plan authoring規約 | prior findings B-001〜B-008-R1/B-005-R1をcanonical planへ修正済み。一本道のadmission、milestone commit/clean/result approval順序、single-writer report統合、S90 dual-role gate、全no-backfill trigger、exact static path、post-commit external evidence境界を補完 | partially_adopted | failed (ChatGPT B-008-R1; fix applied) | yes | assurance再分類、commit / push 後にfresh ChatGPT plan re-review、その後fresh `spec-reviewer` re-review |
