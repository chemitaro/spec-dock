...
| plan | approved requirement/design、ChatGPT planning candidate、Standard assurance obligations、current provider/build/test seams、Issue plan authoring規約 | ChatGPT B-001/B-002とfresh spec-review P1/P2をcanonical planへ修正済み。step-local schema、vertical tracer、Closure/gate/commit contractを補完 | partially_adopted | failed (fresh `spec-reviewer` P1/P2; fixes applied) | yes | assurance再分類、commit / push 後にfresh ChatGPT plan re-review、その後fresh `spec-reviewer` re-review |
