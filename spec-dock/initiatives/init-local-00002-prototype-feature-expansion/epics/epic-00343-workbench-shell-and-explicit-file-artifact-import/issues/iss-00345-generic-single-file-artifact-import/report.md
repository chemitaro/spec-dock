...
| D-023 | resolved | platform / safety | S02 fresh code-reviewer、repo-analyst、consultant、user decision、fresh Epic/Issue spec review | platform共通named-temp cleanupによりLinuxでもfinal pathname check後からunlinkまでにnon-owned entryを削除し得るが、macOS ADRの限定waiverはLinuxへ適用できない | Option A: Linux linkable `O_TMPFILE` anonymous staging必須; Option B: Linuxにもsame-UID waiver; Option C: Linux unsupported; Option D: trusted helper | Option Aを採用。Linuxはanonymous stagingからheld-FD no-replace publicationへ進み、pre-commit failureはFD closeのみとする。capability不足はformal destination前にfail closedし、named-temp fallbackを禁止する | cleanup pathname自体をなくしてriskを構造的に除去し、macOSの限定risk acceptanceを拡張しないため | promoted_to_requirement_design_plan | Epic `artifacts/20260730t102747z-adr-linux-anonymous-staging-trust-boundary.md`; fresh Epic review findings `[]`, confidence 0.99; Issue final fresh review findings `[]`, confidence 0.99 | S02 bounded implementationとfresh code reviewへ移行 |
...
| EAL-005 | adopted | S02 fresh code review、repo-analyst、consultant、user Option A decision、accepted Linux ADR、fresh Epic/Issue spec review | Epic/Issue canonical R/D/PとLinux platform capability contract | Linuxのsame-UID cleanup waiverを不採用とし、linkable `O_TMPFILE` anonymous staging、held-FD publication、pre-commit FD-close-only cleanup、unsupported fail-closedを下降反映した | Epic `artifacts/20260730t102747z-adr-linux-anonymous-staging-trust-boundary.md`; Epic R/D/P/report diff; fresh Epic review findings `[]`, confidence 0.99; Issue final fresh review findings `[]`, confidence 0.99 | S02再実装契約を固定し、assurance valid / execution readyを確認 |
...
| S04 | projection比較が全`updated_at`を除外してmaterial provenance差分を隠す | fresh code-reviewer | root `generated_at`だけ除外し、fixture clock固定。nested `github.updated_at`差分のgenuine Redを確認 | `tc-s04-002`, `CL-AC-014`, `CL-EC-017` | no | S04 full22、unit78、focused3、lint/diff-check pass。fresh rereview待ち |
| S04 | validate/deps/context direct entryをbody-open deny guard下で実行していない | fresh code-reviewer | negative control付きdeny guard下でvalidate_tree/check_deps/sync/context-packを直接実行 | `tc-s04-001`, `tc-s04-003`, `CL-AC-014`, `CL-CON-008` | no | generic body open 0、3-surface focused3 pass。fresh rereview待ち |
...
| consumer | generic body open | projection / authority result |
|---|---:|---|
| validate | 0 | invalid UTF-8でもdecode errorなし |
| sync / ADR mirror | 0 | typed basename gateで除外、mirror差分0 |
| deps / context | 0 | normalized output同値 |
| default authoring discovery | 0 | source/provenance増加なし |

### Compatibility Regression Evidence

| surface | 結果 |
|---|---|
| legacy chatgpt-output CLI / command / presentation | 4 / 8 / 5 passed |
