...
class TestInitUpdate(CliRuntimeHarness):
    ...
    def test_init_creates_expected_structure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("| test id | step | required | evidence level | pre-implementation evidence | verification command | result | notes |", report_text)
    ...
    def test_spec_document_templates_keep_policy_out_of_scaffold(self) -> None:
        ...
        self.assertIn("every bundle `test id` が Central index に存在する", phase_plan_issue)
