...
def test_t02_packaged_candidate_ignores_generated_python_caches(tmp_path: Path) -> None:
    source_assets = Path(__file__).parents[3] / "src" / "spec_dock" / "assets"
    candidate_assets = tmp_path / "assets"
    for _kind, _public_path, source_suffix in FIXED_DOMAINS:
        shutil.copytree(source_assets / source_suffix, candidate_assets / source_suffix, symlinks=True)

    expected = capture_packaged_candidate(candidate_assets)
    generated_cache = candidate_assets / "spec_dock/scripts/spec_dock_runtime/__pycache__"
    generated_cache.mkdir(parents=True, exist_ok=True)
    (generated_cache / "candidate.cpython-312.pyc").write_bytes(b"generated bytecode")
    (candidate_assets / "spec_dock/scripts/generated.pyo").write_bytes(b"generated bytecode")

    assert capture_packaged_candidate(candidate_assets) == expected
