...
def test_report_evidence_gate_accepts_lite_grade_not_applicable_row() -> None:
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| lite | not applicable | not applicable | scope is a small documentation-only change with no specialist need | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | pass | no | execute approved plan | note |\n",
        "lite",
    )
...
def test_report_evidence_gate_rejects_lite_grade_placeholder_reason() -> None:
    _runbook_module, workflow_state = _workflow_modules()
...
def test_report_evidence_gate_accepts_documented_bilingual_spec_authoring_values() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| 要件（requirement） | docs | none | adopted | 合格（passed） | いいえ（no） | 昇格（promote） |\n"
        "| 設計（design） | docs | none | adopted | 合格（passed） | いいえ（no） | 昇格（promote） |\n"
        "| 計画（plan） | docs | none | adopted | 合格（passed） | いいえ（no） | 昇格（promote） |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| 該当なし | 該当なし | 該当なし | 該当なし | 該当なし | not used | [] | not_run | manual authoring | 該当なし | none | pass | execute approved plan |\n\n"
        "#### グレード別専門家証跡ゲート（Grade Specialist Evidence Gate）\n"
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| lite | not applicable | not applicable | scope is a small documentation-only change with no specialist need | 合格（passed） | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | 合格（passed） | no | execute approved plan | note |\n",
        "lite",
    )
...
def test_report_evidence_gate_accepts_localized_adopted_eal_status_references() -> None:
    _runbook_module, workflow_state = _workflow_modules()

    result = workflow_state.evaluate_report_evidence_gate(
        "## 証跡採用台帳（Evidence Adoption Ledger / 必須）\n"
        "| EAL-001 | 部分採用（partially_adopted） | source | design.md | rationale | discussions/draft.md | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| system-architect | iss-00301 | discussions/draft.md | active docs | design.md | 部分採用（partially_adopted） | design.md | orchestrator inspection pass | source input integrated | none | none | pass | execute manual-authored canonical docs |\n\n"
        "#### グレード別専門家証跡ゲート（Grade Specialist Evidence Gate）\n"
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
...
def test_report_evidence_gate_rejects_spec_authoring_placeholder_evidence() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| EAL-001 | adopted | source | design.md | rationale | discussions/draft.md | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | ... | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| 該当なし | 該当なし | 該当なし | 該当なし | 該当なし | not used | [] | not_run | manual authoring | 該当なし | none | pass | execute approved plan |\n\n"
...
def test_report_evidence_gate_rejects_failed_spec_authoring_row_even_with_later_pass() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| EAL-001 | adopted | source | design.md | rationale | discussions/draft.md | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | fail | no | execute approved plan |\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
...
def test_report_evidence_gate_rejects_blocked_fresh_spec_reviewer_promotion() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| standard | system-architect / implementation-planner | skipped | skip reason: existing pattern sufficient | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | pass | no | blocked | blocking finding remains |\n",
        "standard",
    )
...
def test_report_evidence_gate_blocks_short_eal_row_without_status() -> None:
    _runbook_module, workflow_state = _workflow_modules()
...
def test_report_evidence_gate_blocks_eal_template_choice_cells() -> None:
    _runbook_module, workflow_state = _workflow_modules()

    result = workflow_state.evaluate_report_evidence_gate(
        "## 証跡採用台帳（Evidence Adoption Ledger / 必須）\n"
        "| EAL-001 | adopted | sub-agent / reviewer / discussion / command / research | artifact / issue / follow-up | rationale | discussions/draft.md | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
...
def test_report_evidence_gate_blocks_eal_single_path_placeholder() -> None:
    _runbook_module, workflow_state = _workflow_modules()

    result = workflow_state.evaluate_report_evidence_gate(
        "## 証跡採用台帳（Evidence Adoption Ledger / 必須）\n"
        "| EAL-001 | adopted | source | design.md | rationale | path | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
...
def test_report_evidence_gate_blocks_bare_deferred_eal_entries() -> None:
    _runbook_module, workflow_state = _workflow_modules()

    result = workflow_state.evaluate_report_evidence_gate(
        "## 証跡採用台帳（Evidence Adoption Ledger / 必須）\n"
        "| EAL-001 | deferred | source | target | rationale | none | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
...
def test_report_evidence_gate_blocks_scaffolded_eal_entries() -> None:
    _runbook_module, workflow_state = _workflow_modules()

    result = workflow_state.evaluate_report_evidence_gate(
        "## 証跡採用台帳（Evidence Adoption Ledger / 必須）\n"
        "| EAL-001 | adopted | ... | target | rationale | path / command / reviewer finding | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
...
def test_report_evidence_gate_blocks_uppercase_scaffold_marker_in_eal_row() -> None:
    _runbook_module, workflow_state = _workflow_modules()

    result = workflow_state.evaluate_report_evidence_gate(
        "## 証跡採用台帳（Evidence Adoption Ledger / 必須）\n"
        "| EAL-001 | adopted | source | design.md | rationale | AC-___ | none |\n\n"
        "## 仕様 authoring ゲート（Spec Authoring Gate / 必須）\n"
        "| requirement | docs | none | adopted | pass | no | execute approved plan |\n"
        "| design | docs | none | adopted | pass | no | execute approved plan |\n"
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
...
def test_report_evidence_gate_rejects_delegated_draft_placeholder_source_paths() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| system-architect | iss-00301 | discussions/draft.md | ... | design.md | partially_adopted | design.md | orchestrator inspection pass | source input integrated | none | none | pass | execute manual-authored canonical docs |\n\n"
        "#### グレード別専門家証跡ゲート（Grade Specialist Evidence Gate）\n"
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| standard | system-architect / implementation-planner | skipped | skip reason: existing pattern sufficient | pass | ready |\n\n"
...
def test_report_evidence_gate_rejects_delegated_draft_unresolved_blocker_text() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| system-architect | iss-00301 | discussions/draft.md | active docs | design.md | partially_adopted | design.md | orchestrator inspection pass | source input integrated | none | unresolved requirement gap | pass | execute manual-authored canonical docs |\n\n"
        "#### グレード別専門家証跡ゲート（Grade Specialist Evidence Gate）\n"
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| standard | system-architect / implementation-planner | skipped | skip reason: existing pattern sufficient | pass | ready |\n\n"
...
def test_report_evidence_gate_rejects_delegated_draft_failed_diff_guard() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| plan | docs | none | adopted | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| system-architect | iss-00301 | discussions/draft.md | active docs | design.md | partially_adopted | design.md | failed | source input integrated | none | none | pass | execute manual-authored canonical docs |\n\n"
        "#### グレード別専門家証跡ゲート（Grade Specialist Evidence Gate）\n"
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| standard | system-architect / implementation-planner | skipped | skip reason: existing pattern sufficient | pass | ready |\n\n"
...
def test_report_evidence_gate_rejects_empty_standard_skip_reason() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| standard | system-architect / implementation-planner | skipped | skip reason: | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | pass | no | execute approved plan | note |\n",
        "standard",
    )
...
def test_report_evidence_gate_rejects_grade_fallback_template_evidence() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| strict | manual fallback | unavailable | manual fallback evidence | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | pass | no | execute approved plan | note |\n",
        "strict",
    )
...
def test_report_evidence_gate_rejects_waived_reviewer_state_with_pass_marker() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| strict | manual fallback | unavailable | manual authoring fallback with source inspection and residual risk | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | waived (pass) | no | execute approved plan | waiver is not pass |\n",
        "strict",
    )
...
def test_report_evidence_gate_accepts_japanese_manual_authoring_row() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| plan | docs | none | manual authoring candidate | pass | no | execute approved plan |\n\n"
        "## 委任ドラフト証跡（Delegated Draft Evidence / 必須）\n"
        "| 該当なし | 該当なし | 該当なし | 該当なし | 該当なし | not used | [] | not_run | 手動 authoring | 該当なし | none | pass | execute approved plan |\n\n"
        "#### グレード別専門家証跡ゲート（Grade Specialist Evidence Gate）\n"
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| strict | manual fallback | unavailable | manual authoring fallback with source inspection and residual risk | pass | ready |\n\n"
...
def test_report_evidence_gate_accepts_strict_manual_fallback_column_with_manual_evidence() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| strict | system-architect / implementation-planner / manual fallback | unavailable | manual evidence from source inspection and residual risk | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | pass | no | execute approved plan | note |\n",
        "strict",
    )
...
def test_report_evidence_gate_ignores_future_execution_scaffold_choices_after_readiness_sections() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| S01 | 緑フェーズ（Green） | ... | ... | command | pass / fail / blocked | future execution row not filled before implementation |\n\n"
        "### セッションログ（YYYY-MM-DD HH:MM - HH:MM）\n"
        "- AC/EC: AC-___, EC-___\n",
        "standard",
    )
...
def test_report_evidence_gate_rejects_not_applicable_specialist_evidence() -> None:
    _runbook_module, workflow_state = _workflow_modules()
    ...
    result = workflow_state.evaluate_report_evidence_gate(
        ...
        "| Grade | required specialist / fallback | usage | evidence | fresh spec-reviewer verdict | execution readiness |\n"
        "|---|---|---|---|---|---|\n"
        "| strict | system-architect / implementation-planner | used | not applicable | pass | ready |\n\n"
        "#### レビューゲート状態（Reviewer Gate Status）\n"
        "| planning | planning spec-review | spec-reviewer | fresh | pass | no | execute approved plan | note |\n",
        "strict",
    )
