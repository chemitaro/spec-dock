...
- Use `spec-dock/docs/workflow_issue.md` only as the Issue umbrella. Route Issue authoring to `workflow_issue_planning.md` / `spec-dock-issue-planning`, and approved plan execution to `workflow_issue_execution.md` / `spec-dock-issue-execution`.
...
- `spec-dock-issue-planning`: issue-level requirement/design/plan authoring, grill clarification, and fresh Spec Authoring Gate reviewer passes. It does not claim implementation readiness, PR delivery, merge preparation, or issue finish.
- `spec-dock-issue-execution`: issue-level approved plan execution and report updates after fresh Spec Authoring Gate evidence. Planning requests or planning gaps route back to `spec-dock-issue-planning`.
...
- Use one-question-at-a-time for human clarification. Important questions get an unanswered `interview` sheet before asking; specialists return question candidates to the orchestrator instead of asking the user directly.
...
- For Issue authoring, route to `spec-dock-issue-planning`. For concrete dependency, implementation, PR, and completion commands after approved planning, route to `spec-dock-issue-execution` and the reference docs.
