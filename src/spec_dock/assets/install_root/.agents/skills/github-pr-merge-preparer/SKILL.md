...
7. Before fix delegation, pass through the PR Repair Triage Gate:
   - Create or update a PR repair batch from the skill-local template `templates/pr-repair-batch.md` (`pr-repair-batch.md`). Treat `pr-repair-batch.md` as the template source, not the target filename. When an active or explicitly provided writable SpecDock issue scope exists, create or update a timestamped target such as `<ts>-disc-pr-repair-batch.md` under that issue scope's `discussions/` directory as a `disc`; when no writable SpecDock issue scope exists, maintain an inline PR repair batch section in the response or work log using the same template sections and state `batch_path: N/A`.
