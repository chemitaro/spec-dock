...
`delete`、`uninstall --apply`、`worktree remove`、guardを越える`--force`は、正確な対象と破壊的結果が利用者の依頼または承認済み計画に明記されている場合にだけ実行します。PR mergeはrepositoryのhuman gateに従います。これは旧provider固有orchestration surfaceの復活ではなく、外部Codex orchestratorがStorage Core CLIを操作する境界です。
...
- `./spec-dock/scripts/spec-dock update [path]` は repo-local self-update path で、target 省略時は current directory を更新する。明示 path を渡すとその managed repo を更新する。次の固定6ディレクトリを順に丸ごと置換し、全copy成功後に `spec-dock/spec-dock.version` を更新する。配置先のsymlinkや非ディレクトリは書込み前に拒否する:
  `spec-dock/docs`, `spec-dock/templates`, `spec-dock/system`, `spec-dock/scripts`, `.agents/skills/spec-dock`, `.agents/skills/spec-dock-grill-with-docs`。
- updateはnontransactionalです。関連コマンドを停止し、copy failureではnon-zeroで終了します。原因を直した後に外部installerのupdateを最初から再実行してください。journal、自動rollback、resumeはありません。固定6ディレクトリとversion record以外の利用者dataは変更しません。uninstallはツールのみを削除し、`--remove-specs` は拒否します。
...
- `uv run pytest` は選択したテストを通常実行します。専用のpolicy skip、台帳、分割評価器はありません。
