...
- update と recognized target の `init --force` は同じ reconciliation service を使う。unknown / modified / symlink / hard-link / root-rebind は preserve-and-block とし、partial apply は root・intent・contract・plan・protocol と exact pre/postcondition に束縛した `.distribution-journal.json` から同一 package/operation で forward recovery する
