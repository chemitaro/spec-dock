...
- ChatGPT / Oracle は Initiative design / plan と Epic candidate の evidence producer として使える。candidate validation は node creation approval ではないため、Epic node 作成前に人間の明示的承認を通し、採用判断を `report.md` に残す。`local-context` は明示的な lower-authority evidence として扱う
