...
- `artifact import file --epic <id> --file <path>` は、Workbench を要求せず、一件の明示 regular file を generic Artifact として保存します。source は変更せず、bytes は opaque evidence のままです。`canonical=false` は自動採用しないことを表します。命名は [reference_naming.md](../../reference_naming.md)、privacy-safe result と publication / retry state は [guide.md](../../guide.md) を参照してください。
...
- Unsupported issue-only draft artifact types:
  - `draft-requirement`: Epic scope では unsupported です。future `new artifact draft-requirement --epic` は write 前に no-write fail-closed します。
  - `draft-design`: Epic scope では unsupported です。future `new artifact draft-design --epic` は write 前に no-write fail-closed します。
  - `draft-plan`: Epic scope では unsupported です。future `new artifact draft-plan --epic` は write 前に no-write fail-closed します。
