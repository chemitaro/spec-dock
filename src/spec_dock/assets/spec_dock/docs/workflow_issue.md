...
- ChatGPT-first Issue planning route は、非自明な Issue planning の primary evidence-production route として使う。Issue Planning は単一 workflow であり、`requirement-heavy`、`draft-heavy`、`context-heavy` は入力 context framing であって別 workflow mode ではない。`spec-dock-issue-planning-manual` は human-approved emergency backup であり、hard / unrecoverable ChatGPT route failure と recovery attempts、explicit approval、fresh reviewer gate の evidence がある場合だけ使う
